...
function emptyPreview() {
  return { name:"", hostType:"", tagline:"", shortDescription:"", description:"", city:"", state:"", websiteUrl:"", publicEmail:"", phone:"", instagramUrl:"", facebookUrl:"", specialties:[] as string[], serviceAreas:[] as string[], audiences:[] as string[], languages:[] as string[], accessibility:"", foundedYear:null as number|null, confidenceNotes:[] as string[] };
}
function unique(values: string[]) { return [...new Set(values.map((value) => value.trim()).filter(Boolean))]; }
function findLabeled(text: string, labels: string[]) { for (const label of labels) { const match=text.match(new RegExp(`(?:^|\\n)\\s*${label}\\s*[:\\-]\\s*([^\\n]+)`,"i")); if(match?.[1])return clean(match[1],1000); } return ""; }
...
function schema() {
  return { type:"object", additionalProperties:false, required:["name","hostType","tagline","shortDescription","description","city","state","websiteUrl","publicEmail","phone","instagramUrl","facebookUrl","specialties","serviceAreas","audiences","languages","accessibility","foundedYear","confidenceNotes"], properties:{
    name:{type:"string"}, hostType:{type:"string",enum:["","individual","business","organization","nonprofit","community","venue","creator","other"]}, tagline:{type:"string"}, shortDescription:{type:"string"}, description:{type:"string"}, city:{type:"string"}, state:{type:"string"}, websiteUrl:{type:"string"}, publicEmail:{type:"string"}, phone:{type:"string"}, instagramUrl:{type:"string"}, facebookUrl:{type:"string"}, specialties:{type:"array",maxItems:20,items:{type:"string"}}, serviceAreas:{type:"array",maxItems:20,items:{type:"string"}}, audiences:{type:"array",maxItems:20,items:{type:"string"}}, languages:{type:"array",maxItems:20,items:{type:"string"}}, accessibility:{type:"string"}, foundedYear:{type:["integer","null"]}, confidenceNotes:{type:"array",maxItems:20,items:{type:"string"}}
  }};
}
...
function normalizePreview(raw:any){
  const preview=emptyPreview();const hostTypes=new Set(["individual","business","organization","nonprofit","community","venue","creator","other"]);
  preview.name=clean(raw?.name,160);preview.hostType=hostTypes.has(raw?.hostType)?raw.hostType:"";preview.tagline=clean(raw?.tagline,180);preview.shortDescription=clean(raw?.shortDescription,280);preview.description=clean(raw?.description,6000);preview.city=clean(raw?.city,120);preview.state=clean(raw?.state,80);preview.websiteUrl=clean(raw?.websiteUrl,500);preview.publicEmail=clean(raw?.publicEmail,320);preview.phone=clean(raw?.phone,120);preview.instagramUrl=clean(raw?.instagramUrl,500);preview.facebookUrl=clean(raw?.facebookUrl,500);
  preview.specialties=unique(Array.isArray(raw?.specialties)?raw.specialties.map((value:unknown)=>clean(value,120)):[]).slice(0,20);preview.serviceAreas=unique(Array.isArray(raw?.serviceAreas)?raw.serviceAreas.map((value:unknown)=>clean(value,120)):[]).slice(0,20);preview.audiences=unique(Array.isArray(raw?.audiences)?raw.audiences.map((value:unknown)=>clean(value,120)):[]).slice(0,20);preview.languages=unique(Array.isArray(raw?.languages)?raw.languages.map((value:unknown)=>clean(value,120)):[]).slice(0,20);preview.accessibility=clean(raw?.accessibility,3000);
  const foundedYear=Number(raw?.foundedYear);preview.foundedYear=Number.isInteger(foundedYear)&&foundedYear>=1800&&foundedYear<=2200?foundedYear:null;preview.confidenceNotes=unique(Array.isArray(raw?.confidenceNotes)?raw.confidenceNotes.map((value:unknown)=>clean(value,500)):[]).slice(0,20);return preview;
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const {data:userData,error:userError}=await client.auth.getUser();const userId=userData.user?.id;if(userError||!userId)return json({error:"Authentication required"},401);
    const body=await req.json();const organizationId=clean(body?.organizationId,80),mode=clean(body?.mode,30);
    if(!organizationId||!["files","website","pasted_text"].includes(mode))return json({error:"Invalid profile import request."},400);
    const {data:membership,error:membershipError}=await client.from("host_organization_members").select("role").eq("organization_id",organizationId).eq("profile_id",userId).maybeSingle();
    if(membershipError)throw membershipError;if(!membership||!["owner","admin"].includes(membership.role))return json({error:"Owner or admin access is required."},403);

    const inputContent:any[]=[],textSources:string[]=[];let sourceLabel="Profile details",sourceUrl:string|null=null;
    ...
    if(mode==="website"){
      ...
      const html=(await response.text()).slice(0,250000),text=htmlToText(html);if(!text)return json({error:"No readable public profile details were found on that page."},400);textSources.push(text);sourceLabel=parsed.hostname;
    }
    if(mode==="files"){
      const files=Array.isArray(body?.files)?body.files.slice(0,MAX_FILES):[];if(!files.length)return json({error:"Upload at least one profile source file."},400);if(Array.isArray(body?.files)&&body.files.length>MAX_FILES)return json({error:`Upload up to ${MAX_FILES} files at a time.`},400);
      sourceLabel=files.length===1?clean(files[0]?.name,240)||"Uploaded profile file":`${files.length} uploaded profile files`;
      for(const raw of files){const path=clean(raw?.path,1000),name=clean(raw?.name,240);if(!path||!name||!safePath(path,userId))return json({error:"One uploaded file path is invalid."},400);const mime=mimeFor(name,clean(raw?.mimeType,160));const {data:blob,error:downloadError}=await client.storage.from("event-imports").download(path);if(downloadError||!blob)throw downloadError??new Error(`Unable to read ${name}.`);const bytes=new Uint8Array(await blob.arrayBuffer());if(bytes.byteLength>MAX_FILE_BYTES)return json({error:`${name} is larger than 10 MB.`},400);
    ...
    const combinedText=textSources.join("\n\n--- SOURCE ---\n\n").slice(0,80000);let extractionSource:"ai"|"source"|"fallback"="fallback",preview=fallbackFromText(combinedText);
    if(openAiKey&&(combinedText||inputContent.length)){
      if(combinedText)inputContent.unshift({type:"input_text",text:`SOURCE MATERIAL\n${combinedText}`});
      const upstream=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${openAiKey}`},body:JSON.stringify({model:MODEL,instructions:"Build a proposed public host profile from the supplied business, organization, creator, venue, nonprofit, community, or individual-host materials. Extract only facts directly supported by the sources. Never invent a location, founding year, service area, audience, accessibility feature, contact detail, social URL, specialty, credential, or description. Keep description factual rather than promotional unless the source itself is promotional. shortDescription should be at most 220 characters. tagline should be at most 120 characters. Normalize a clearly stated US state to its two-letter abbreviation. Use hostType only when the source clearly supports it, otherwise return an empty string. A service area means where the host operates, not every location merely mentioned. Add confidenceNotes for conflicts, uncertain details, or fields that need human review. The user will review every proposed change before it is applied.",input:[{role:"user",content:[{type:"input_text",text:"Extract proposed host profile fields from these sources."},...inputContent]}],text:{format:{type:"json_schema",name:"host_profile_import_preview",strict:true,schema:schema()}}})});
      const payload=await upstream.json();if(upstream.ok){const output=readOutputText(payload);if(output){try{preview=normalizePreview(JSON.parse(output));extractionSource="ai";}catch{extractionSource=combinedText?"source":"fallback";}}}else{console.error("host-profile-import-preview upstream",JSON.stringify(payload).slice(0,4000));extractionSource=combinedText?"source":"fallback";}
    }else if(combinedText)extractionSource="source";else if(inputContent.length)return json({error:"Image and PDF profile reading is not configured on the server yet."},503);

    const {data:importRow,error:importError}=await client.from("host_profile_imports").insert({organization_id:organizationId,owner_profile_id:userId,source_type:mode,source_label:sourceLabel,source_url:sourceUrl,extracted_payload:preview,approved_payload:{},status:"preview"}).select("id").single();if(importError)throw importError;
    return json({importId:importRow.id,sourceLabel,extractionSource,preview});
  }catch(error){console.error("host-profile-import-preview",error);return json({error:error instanceof Error?error.message:"Unable to read these host profile details."},500);}
});
