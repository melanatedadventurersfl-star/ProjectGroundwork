...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: photo, error: photoError } = await userClient
      ...
      .select("id,profile_id,storage_path,caption,moderation_status")
      .eq("id", photoId)
      .single();
    if (photoError || !photo) return json({ error: "Photo not found or not accessible." }, 404);
    if (photo.profile_id !== userData.user.id) {
      const { data: isAdmin } = await userClient.rpc("is_platform_admin");
      if (isAdmin !== true) return json({ error: "Photo not found or not accessible." }, 404);
    }
    if (photo.moderation_status !== "pending") return json({ status: photo.moderation_status, alreadyModerated: true });

    if (!openAiKey) {
      await adminClient.from("trail_guide_photos").update({
    ...
    if (signedError || !signed?.signedUrl) {
      await adminClient.from("trail_guide_photos").update({
        moderation_source: "ai",
        moderation_model: MODEL,
        moderation_reason: "Automated moderation could not access the stored photo. Human review is required.",
      }).eq("id", photo.id);
    ...
    input.push({ type: "image_url", image_url: { url: signed.signedUrl } });
    ...
    if (!upstream.ok) {
      await adminClient.from("trail_guide_photos").update({
        moderation_source: "ai",
        moderation_model: MODEL,
        moderation_reason: "Automated moderation could not complete. Human review is required.",
      }).eq("id", photo.id);
    ...
    if (result.flagged === true) {
      ...
      status = "pending";
      reason = "Automated moderation found an ambiguous safety signal. Human review is required.";
    } else {
      status = "approved";
      reason = "Automatically approved by safety moderation.";
    }

    const { error: updateError } = await adminClient.from("trail_guide_photos").update({
    ...
    console.error("moderate-trail-guide-photo", error);
