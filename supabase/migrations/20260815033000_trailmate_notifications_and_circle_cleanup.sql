...
drop trigger if exists member_connections_notify_trailmates on public.member_connections;
create trigger member_connections_notify_trailmates
after insert or update of status on public.member_connections
for each row execute function public.notify_trailmate_connection_change();
...
drop trigger if exists member_connections_cleanup_circles on public.member_connections;
create trigger member_connections_cleanup_circles
after delete on public.member_connections
for each row execute function public.cleanup_circle_membership_after_connection_delete();
