-- Host identity community routing V2
-- Keeps tenant ownership and public host identity ownership separate.
...
-- Existing same-name host communities belong to that public host identity.
...
-- Go Melanated has legacy duplicate MA host identities. Use the canonical
-- Melanated Adventurers identity already used by the current product language.
with canonical_host as (
  select
...
set primary_host_organization_id = c.host_organization_id,
    updated_at = now()
from canonical_host c
where o.id = c.organization_id
  and c.host_organization_id is not null;
...
-- Create the canonical host community when the public identity does not yet
-- have one. Existing topic-room memberships are not copied.
...
-- Resolve a primary community only when the host identity has one unambiguous
-- candidate or one exact name match.
with candidates as (
  select
    h.id as host_organization_id,
    (array_agg(
      g.id
      order by
...
-- The public host identity owner belongs to its primary community as a host.
...
update public.community_group_members gm
set role = 'host'
from public.host_organizations h
where h.primary_community_id = gm.group_id
  and h.created_by = gm.profile_id
  and gm.role = 'member';

-- Tenant-wide primary community routing is deprecated. Public host identity is
-- now authoritative.
...
-- Compatibility RPCs delegate to the active tenant's default public host identity.
...
-- Automatic event routing follows the event's public host identity, while
-- platform_organization_id remains the tenant boundary.
...
begin
  ...
  if tg_op = 'UPDATE' and old.platform_organization_id is distinct from new.platform_organization_id then
    delete from public.community_outings
    where adventure_id = new.id;
  else
    delete from public.community_outings
...
-- Remove tenant-wide primary links and rebuild only from explicit public host
-- identities. Deliberate non-primary shares remain intact.
delete from public.community_outings
where is_primary;
