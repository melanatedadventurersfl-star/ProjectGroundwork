-- Repair deployment drift for the platform-level new member review queue.
-- This migration is intentionally idempotent so environments that missed the
-- original queue migration can recover without changing the client contract.

create table if not exists public.member_signup_reviews (
...
create index if not exists member_signup_reviews_unreviewed_idx
...
-- Recover signups that arrived while the queue/RPC was unavailable.
insert into public.member_signup_reviews (profile_id, created_at)
...
-- Make PostgREST refresh function metadata immediately after this migration.
