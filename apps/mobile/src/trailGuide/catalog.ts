...
function collectionsFor(city: TrailGuideCityKey, category: Exclude<DiscoveryCategory, 'All'>, area: string) {
  ...
  const localToken = localLabel.split(' ')[0] ?? localLabel;
  const localCollection = cityCollections[city][0] ?? 'Close By';
  ...
  if (area.toLowerCase().includes(localToken)) collections.push(localCollection);
