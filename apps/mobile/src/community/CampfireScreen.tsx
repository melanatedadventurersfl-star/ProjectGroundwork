...
function relativeTime(value: string) {
  ...
  return days < 7 ? `${days}d ago` : created.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
}
...
function postTypeLabel(post: CommunityPost) {
  ...
  if (post.post_type === 'recommendation') return 'PLACE';
...
function getActiveToken(body: string, cursor: number): ActiveToken {
  ...
  const query = match[3] ?? '';
  const start = cursor - trigger.length - query.length;
  return { trigger, query, start, end: cursor };
}

function metadataPeople(metadata: Record<string, unknown>) {
  const value = metadata.tagged_people;
...
function metadataGroups(metadata: Record<string, unknown>) {
  const value = metadata.tagged_groups;
  return Array.isArray(value) ? value.filter((item): item is { id: string; name: string } => {
    if (!item || typeof item !== 'object') return false;
    const candidate = item as Record<string, unknown>;
    return typeof candidate.id === 'string' && typeof candidate.name === 'string';
...
function RichPostBody({ post }: { post: CommunityPost }) {
  const people = metadataPeople(post.metadata);
  const groups = metadataGroups(post.metadata);
  const hashtags = metadataHashtags(post.metadata);
  const tokens = [
    ...people.map((item) => ({ text: `@${item.name}`, type: 'person' as const, id: item.id })),
    ...groups.map((item) => ({ text: `&${item.name}`, type: 'group' as const, id: item.id })),
    ...hashtags.map((tag) => ({ text: `#${tag}`, type: 'hashtag' as const, id: tag })),
  ...
  while (index < post.body.length) {
    const matches = tokens
      .map((token) => ({ token, at: post.body.indexOf(token.text, index) }))
      .filter((item) => item.at >= 0)
      .sort((a, b) => a.at - b.at || b.token.text.length - a.token.text.length);
    const next = matches[0];
    if (!next) {
      pieces.push({ text: post.body.slice(index) });
      break;
    }
  ...
  return (
    <Text style={styles.feedBody}>
      {pieces.map((piece, pieceIndex) => piece.token ? (
        <Text
          key={`${pieceIndex}-${piece.text}`}
          style={styles.inlineToken}
          onPress={() => {
        ...
        >{piece.text}</Text>
      ) : <Text key={`${pieceIndex}-${piece.text}`}>{piece.text}</Text>)}
    </Text>
  );
}
  ...
  return (
    ...
    >
      <View style={styles.groupAvatar}>
        {group.image_url ? <Image source={{ uri: group.image_url }} style={styles.avatarImage} /> : <Text style={styles.groupAvatarText}>{initials(group.name)}</Text>}
      </View>
      <View style={styles.flex}>
        <Text style={styles.groupName} numberOfLines={1}>{group.name}</Text>
        <Text style={styles.groupMeta} numberOfLines={1}>
          {group.is_member ? `${group.member_count} member${group.member_count === 1 ? '' : 's'}` : joining ? 'Joining…' : `${group.member_count} members · Tap to join`}
        </Text>
      </View>
  ...
  return (
    <Pressable style={({ pressed }) => [styles.personChip, pressed && styles.pressed]} onPress={() => router.push({ pathname: '/community-profile/[id]', params: { id: person.id } })}>
      <View style={styles.personAvatar}>
        {person.avatar_url ? <Image source={{ uri: person.avatar_url }} style={styles.avatarImage} /> : <Text style={styles.personAvatarText}>{initials(person.display_name)}</Text>}
      </View>
    ...
    <Pressable style={({ pressed }) => [styles.feedCard, pressed && styles.pressed]} onPress={() => router.push(`/community/${post.id}`)}>
      <View style={styles.feedHeader}>
        <View style={styles.feedAvatar}>
          {post.avatar_url ? <Image source={{ uri: post.avatar_url }} style={styles.avatarImage} /> : <Text style={styles.feedAvatarText}>{initials(post.author_name)}</Text>}
        </View>
        <View style={styles.flex}>
...
export default function CampfireScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const [nextGroups, nextCrews, nextPosts, basecamp, nextMembers] = await Promise.all([
        getGroups(),
        getCircles(),
        getCommunityFeed(),
        getMemberBasecamp(),
  ...
  const nearbyGroups = useMemo(() => groups.filter((group) => {
    if (!homeState || group.state !== homeState) return false;
    return !homeCity || !group.city || group.city === homeCity;
  }), [groups, homeCity, homeState]);
  const nearbyPeople = useMemo(() => members.filter((person) => {
    if (!homeState || person.home_state !== homeState) return false;
    return !homeCity || !person.home_city || person.home_city === homeCity;
  }), [members, homeCity, homeState]);
  ...
  const localRecommendations = useMemo(() => posts.filter((post) => {
    if (post.post_type !== 'recommendation') return false;
    const location = typeof post.metadata.place_location === 'string' ? post.metadata.place_location.toLowerCase() : '';
    return (homeCity && location.includes(homeCity.toLowerCase())) || nearbyIds.has(post.author_id);
  }).slice(0, 4), [posts, homeCity, nearbyIds]);
  ...
  const groupSuggestions = useMemo(() => {
    ...
    return groups.filter((group) => !needle || group.name.toLowerCase().includes(needle)).slice(0, 7);
  }, [activeToken, groups]);
  const hashtagSuggestions = useMemo(() => {
    if (activeToken?.trigger !== '#') return [];
    const needle = activeToken.query.toLowerCase();
    const matches = popularHashtags.filter((tag) => !needle || tag.includes(needle));
    if (needle && !matches.includes(needle)) return [needle, ...matches].slice(0, 8);
    return matches.slice(0, 8);
  }, [activeToken, popularHashtags]);

  useEffect(() => {
    if (activeToken?.trigger !== '@' || activeToken.query.trim().length < 2) {
      setMentionResults([]);
    ...
    const timer = setTimeout(() => {
      ...
      void searchCommunityMembers(activeToken.query)
        .then((results) => { if (!cancelled) setMentionResults(results.slice(0, 8)); })
        .catch(() => { if (!cancelled) setMentionResults([]); })
        .finally(() => { if (!cancelled) setMentionSearching(false); });
    }, 180);
  ...
  function replaceActiveToken(replacement: string) {
    ...
    const nextCursor = activeToken.start + replacement.length + 1;
  ...
  function choosePerson(person: CommunityPerson) {
    ...
    if (!postTagging.people.some((item) => item.id === person.id)) {
      setPostTagging((current) => ({ ...current, people: [...current.people, { id: person.id, display_name: person.display_name, avatar_url: person.avatar_url }] }));
    }
  }

  function chooseGroup(group: CommunityGroup) {
    replaceActiveToken(`&${group.name}`);
    if (!postTagging.groups.some((item) => item.id === group.id)) {
      setPostTagging((current) => ({ ...current, groups: [...current.groups, { id: group.id, name: group.name }] }));
    }
  }
  ...
  async function submitPost() {
    if (composerType === 'meetup') {
      ...
      router.push({ pathname: '/local-events/create', params: { audience: composerAudience, circleId: composerCircleId ?? undefined, groupId: composerGroupId ?? undefined } });
      return;
    }
  ...
  const showSuggestions = !!activeToken;
        ...
        {tab === 'campfire' ? (
            ...
            <View style={styles.composer}>
              {composerPhoto ? (
                <View style={styles.composerPhotoWrap}>
                  <Image source={{ uri: composerPhoto.uri }} style={styles.composerPhoto} />
                  <Pressable style={styles.removePhotoButton} onPress={() => setComposerPhoto(null)}><Ionicons name="close" size={18} color={TEXT} /></Pressable>
                </View>
              ) : null}

              <TextInput
                value={composerBody}
                onChangeText={(text) => { setComposerBody(text); setCursor(text.length); }}
              ...
              {showSuggestions ? (
                <View style={styles.suggestionPanel}>
                  <View style={styles.suggestionHeading}>
                    <Text style={styles.suggestionTitle}>{activeToken?.trigger === '@' ? 'People' : activeToken?.trigger === '&' ? 'Groups' : 'Topics'}</Text>
                    <Text style={styles.suggestionHint}>{activeToken?.trigger === '@' ? '@' : activeToken?.trigger === '&' ? '&' : '#'}{activeToken?.query}</Text>
                  </View>
                  {activeToken?.trigger === '@' ? (
                    <>
                      {activeToken.query.length < 2 ? <Text style={styles.suggestionEmpty}>Type 2 letters to search Trailmates.</Text> : null}
                      {mentionSearching ? <ActivityIndicator color={GOLD} style={styles.suggestionLoader} /> : null}
                      {mentionResults.map((person) => (
                        <Pressable key={person.id} style={styles.suggestionRow} onPress={() => choosePerson(person)}>
                          <View style={styles.suggestionAvatar}>{person.avatar_url ? <Image source={{ uri: person.avatar_url }} style={styles.avatarImage} /> : <Text style={styles.suggestionAvatarText}>{initials(person.display_name)}</Text>}</View>
                          <View style={styles.flex}><Text style={styles.suggestionName}>{person.display_name}</Text><Text style={styles.suggestionMeta}>{[person.home_city, person.home_state].filter(Boolean).join(', ') || 'Member'}</Text></View>
                          <Text style={styles.triggerBadge}>@</Text>
                        </Pressable>
                      ))}
                    </>
                  ) : activeToken?.trigger === '&' ? (
                    <>
                      {groupSuggestions.map((group) => (
                        <Pressable key={group.id} style={styles.suggestionRow} onPress={() => chooseGroup(group)}>
                          <View style={styles.suggestionAvatar}><Text style={styles.suggestionAvatarText}>{initials(group.name)}</Text></View>
                          <View style={styles.flex}><Text style={styles.suggestionName}>{group.name}</Text><Text style={styles.suggestionMeta}>{group.member_count} members</Text></View>
                          <Text style={styles.triggerBadge}>&</Text>
                        </Pressable>
                      ))}
                      {!groupSuggestions.length ? <Text style={styles.suggestionEmpty}>No groups found.</Text> : null}
                    </>
                  ) : (
                    <>
                      {hashtagSuggestions.map((tag) => (
                        <Pressable key={tag} style={styles.hashtagRow} onPress={() => chooseHashtag(tag)}><Text style={styles.hashtagText}>#{tag}</Text></Pressable>
                      ))}
                      {!hashtagSuggestions.length && !activeToken?.query ? <Text style={styles.suggestionEmpty}>Start typing a topic.</Text> : null}
                    </>
                  )}
                </View>
              ) : null}
              ...
              <View style={styles.composerControls}>
                <Pressable style={styles.compactControl} onPress={() => void choosePhoto()}><Ionicons name="image-outline" size={18} color={GOLD_MUTED} /><Text style={styles.compactControlText}>{composerPhoto ? 'Change photo' : 'Photo'}</Text></Pressable>
                <Pressable style={styles.compactControl} onPress={() => { setTypeOpen((value) => !value); setAudienceOpen(false); setTargetOpen(false); }}><Ionicons name={selectedType.icon as never} size={18} color={GOLD_MUTED} /><Text style={styles.compactControlText}>{selectedType.label}</Text><Ionicons name={typeOpen ? 'chevron-up' : 'chevron-down'} size={15} color={MUTED} /></Pressable>
              </View>
              ...
              <View style={styles.composerFooter}>
                <Pressable style={styles.shareControl} onPress={() => { setAudienceOpen((value) => !value); setTypeOpen(false); setTargetOpen(false); }}><Ionicons name={selectedAudience.icon as never} size={16} color={GOLD} /><Text style={styles.shareText}>Share with {selectedAudience.label}</Text><Ionicons name={audienceOpen ? 'chevron-up' : 'chevron-down'} size={14} color={MUTED} /></Pressable>
                <Pressable disabled={cannotPost} style={[styles.postButton, cannotPost && styles.postButtonDisabled]} onPress={() => void submitPost()}>{submitting ? <ActivityIndicator size="small" color="#101510" /> : <Text style={styles.postButtonText}>{composerType === 'meetup' ? 'Set up' : 'Post'}</Text>}</Pressable>
              </View>
              ...
              {(composerAudience === 'circle' || composerAudience === 'group') ? (
                <View>
                  <Pressable style={styles.targetControl} onPress={() => setTargetOpen((value) => !value)}><Text style={styles.targetControlText}>{composerAudience === 'circle' ? selectedCrew?.name ?? 'Choose a Crew' : selectedGroup?.name ?? 'Choose a Group'}</Text><Ionicons name={targetOpen ? 'chevron-up' : 'chevron-down'} size={15} color={MUTED} /></Pressable>
                  {targetOpen ? <View style={styles.targetList}>{(composerAudience === 'circle' ? crews : yourGroups).map((target) => <Pressable key={target.id} style={styles.dropdownRow} onPress={() => { if (composerAudience === 'circle') setComposerCircleId(target.id); else setComposerGroupId(target.id); setTargetOpen(false); }}><Text style={styles.dropdownText}>{target.name}</Text></Pressable>)}</View> : null}
                </View>
              ) : null}
            </View>
          ...
          <>
            <View style={styles.nearbyHero}>
              <View style={styles.nearbyHeroIcon}><Ionicons name="navigate" size={22} color="#101510" /></View>
              <View style={styles.flex}><Text style={styles.nearbyEyebrow}>YOUR LOCAL PULSE</Text><Text style={styles.nearbyTitle}>Near {locationLabel}</Text><Text style={styles.nearbyCopy}>{nearbyPeople.length} adventurer{nearbyPeople.length === 1 ? '' : 's'} · {nearbyGroups.length} group{nearbyGroups.length === 1 ? '' : 's'} nearby</Text></View>
            </View>
            ...
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Adventurers Around You</Text><Text style={styles.sectionSubheading}>People with the same local basecamp.</Text></View><Ionicons name="people-outline" size={20} color={GOLD_MUTED} /></View>
            ...
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Happening Nearby</Text><Text style={styles.sectionSubheading}>Meetups and adventure-buddy posts from local members.</Text></View><Ionicons name="calendar-outline" size={20} color={GOLD_MUTED} /></View>
            ...
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Local Campfire</Text><Text style={styles.sectionSubheading}>Recent posts from adventurers around {locationLabel}.</Text></View><Ionicons name="bonfire-outline" size={20} color={GOLD_MUTED} /></View>
            {localPosts.slice(0, 5).map((post) => <CommunityPostCard key={`local-${post.id}`} post={post} />)}
            {!localPosts.length ? <Text style={styles.inlineEmpty}>Local posts will show here as the Campfire grows.</Text> : null}

            <View style={styles.sectionCard}>
              <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Groups Near You</Text><Text style={styles.sectionSubheading}>Local communities you can jump into.</Text></View><Ionicons name="people-circle-outline" size={20} color={GOLD_MUTED} /></View>
              <View style={styles.groupList}>{nearbyGroups.map((group) => <GroupRow key={group.id} group={group} joining={joiningId === group.id} onJoin={(next) => void handleJoin(next)} />)}{!nearbyGroups.length ? <Text style={styles.emptyText}>No local groups yet.</Text> : null}</View>
            </View>

            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Explore Nearby</Text><Text style={styles.sectionSubheading}>Places the community thinks are worth getting outside for.</Text></View><Ionicons name="map-outline" size={20} color={GOLD_MUTED} /></View>
            {localRecommendations.map((post) => <CommunityPostCard key={`place-${post.id}`} post={post} />)}
            {!localRecommendations.length ? <Text style={styles.inlineEmpty}>Nearby recommendations will collect here.</Text> : null}
          </>
        ) : tab === 'crew' ? (
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Your Crew</Text><Text style={styles.sectionSubheading}>Your Trailmates, organized your way.</Text></View><Pressable onPress={() => router.push('/circles')}><Text style={styles.link}>Manage</Text></Pressable></View>
            <View style={styles.groupList}>{crews.map((crew) => <CrewRow key={crew.id} crew={crew} />)}{!crews.length ? <Text style={styles.emptyText}>Create a crew to organize the Trailmates you adventure with most.</Text> : null}</View>
          </View>
        ) : (
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Groups</Text><Text style={styles.sectionSubheading}>Adventure communities you belong to or can discover.</Text></View><Ionicons name="albums-outline" size={20} color={GOLD_MUTED} /></View>
            <View style={styles.groupList}>{groups.map((group) => <GroupRow key={group.id} group={group} joining={joiningId === group.id} onJoin={(next) => void handleJoin(next)} />)}{!groups.length ? <Text style={styles.emptyText}>No groups yet.</Text> : null}</View>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  content: { paddingHorizontal: 18, paddingTop: 14, paddingBottom: 48, gap: 12 },
  flex: { flex: 1 },
  ...
  composerInput: { minHeight: 70, maxHeight: 170, color: TEXT, fontSize: 15, lineHeight: 21, paddingHorizontal: 3, paddingVertical: 5, textAlignVertical: 'top' },
  ...
  suggestionPanel: { borderWidth: 1, borderColor: '#455349', borderRadius: 13, overflow: 'hidden', backgroundColor: '#111A16' },
  suggestionHeading: { minHeight: 35, paddingHorizontal: 10, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#344239' },
  suggestionTitle: { color: TEXT, fontSize: 11.5, fontWeight: '900' },
  suggestionHint: { color: GOLD, fontSize: 11.5, fontWeight: '800' },
  suggestionRow: { minHeight: 51, flexDirection: 'row', alignItems: 'center', gap: 9, paddingHorizontal: 9, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#2E3A33' },
  suggestionAvatar: { width: 34, height: 34, borderRadius: 17, overflow: 'hidden', backgroundColor: '#263A2F', borderWidth: 1, borderColor: '#4A594F', alignItems: 'center', justifyContent: 'center' },
  suggestionAvatarText: { color: TEXT, fontWeight: '900', fontSize: 10.5 },
  suggestionName: { color: TEXT, fontSize: 12.5, fontWeight: '800' },
  suggestionMeta: { color: MUTED, fontSize: 10.5, marginTop: 1 },
  suggestionEmpty: { color: MUTED, padding: 11, fontSize: 11.5 },
  suggestionLoader: { marginVertical: 10 },
  triggerBadge: { color: GOLD, fontSize: 18, fontWeight: '900', width: 22, textAlign: 'center' },
  hashtagRow: { minHeight: 40, justifyContent: 'center', paddingHorizontal: 11, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#2E3A33' },
  hashtagText: { color: GOLD, fontWeight: '850', fontSize: 13 },
  ...
  postButton: { minWidth: 72, minHeight: 38, borderRadius: 11, paddingHorizontal: 13, backgroundColor: GOLD, alignItems: 'center', justifyContent: 'center' },
  ...
  feedCard: { backgroundColor: 'transparent', paddingHorizontal: 2, paddingVertical: 10, gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#26332C' },
  ...
  feedAvatarText: { color: TEXT, fontWeight: '900', fontSize: 11.5 },
  ...
  feedBody: { color: '#E0E5E1', fontSize: 13.5, lineHeight: 20 },
  inlineToken: { color: GOLD, fontWeight: '800' },
  ...
  sectionTitle: { color: TEXT, fontSize: 17.5, fontWeight: '900' },
  sectionSubheading: { color: '#8F9B93', fontSize: 11.5, lineHeight: 16, marginTop: 2 },
  ...
  groupAvatar: { width: 38, height: 38, borderRadius: 19, overflow: 'hidden', borderWidth: 1, borderColor: '#4A594F', backgroundColor: '#1D3026', alignItems: 'center', justifyContent: 'center' },
  ...
  groupAvatarText: { color: TEXT, fontWeight: '900', fontSize: 10 },
  groupName: { color: TEXT, fontWeight: '800', fontSize: 13.5 },
  groupMeta: { color: '#8F9B93', fontSize: 11, marginTop: 2 },

  nearbyHero: { backgroundColor: '#1C2A23', borderWidth: 1, borderColor: '#4A5A4F', borderRadius: 18, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 11 },
  nearbyHeroIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: GOLD, alignItems: 'center', justifyContent: 'center' },
  nearbyEyebrow: { color: '#AE9964', fontSize: 9.5, fontWeight: '900', letterSpacing: 1 },
  nearbyTitle: { color: TEXT, fontSize: 20, fontWeight: '900', marginTop: 1 },
  nearbyCopy: { color: MUTED, fontSize: 11.5, marginTop: 3 },
  makePlanButton: { minHeight: 44, backgroundColor: GOLD, borderRadius: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 },
  makePlanText: { color: '#101510', fontWeight: '900', fontSize: 13 },
  peopleRow: { gap: 10, paddingVertical: 2, paddingRight: 10 },
  personChip: { width: 67, alignItems: 'center', gap: 5 },
  personAvatar: { width: 54, height: 54, borderRadius: 27, overflow: 'hidden', backgroundColor: '#294236', borderWidth: 1.5, borderColor: '#6D795F', alignItems: 'center', justifyContent: 'center' },
  personAvatarText: { color: TEXT, fontWeight: '900', fontSize: 11 },
  personChipName: { color: '#DDE2DF', fontWeight: '800', fontSize: 10.5, maxWidth: 64 },
  inlineEmpty: { color: '#87938B', fontSize: 12, lineHeight: 18, paddingVertical: 5 },

  emptyCard: { backgroundColor: CARD, borderWidth: 1, borderColor: BORDER, borderRadius: 16, padding: 18, alignItems: 'center', gap: 6 },
  emptyTitle: { color: TEXT, fontWeight: '900', fontSize: 15 },
