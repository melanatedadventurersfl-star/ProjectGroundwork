...
function CommunityPostCard({ post }: { post: CommunityPost }) {
  ...
  const audienceMeta = post.audience === 'connections'
    ? { icon: 'people-outline', label: 'Trailmates' }
    : post.audience === 'circle'
      ? { icon: 'people-circle-outline', label: 'Crew' }
      : post.audience === 'group'
        ? { icon: 'albums-outline', label: 'Group' }
        : null;
    ...
    <Pressable style={({ pressed }) => [styles.feedCard, pressed && styles.pressed]} onPress={() => router.push(`/community/${post.id}`)}>
      <View style={styles.feedHeader}>
        <Pressable
          ...
          hitSlop={6}
          onPress={(event) => { event.stopPropagation(); openAuthorProfile(); }}
          style={({ pressed }) => [styles.authorProfileTarget, pressed && styles.authorProfileTargetPressed]}
        >
          ...
          <View style={styles.authorCopy}>
            <View style={styles.authorLine}><Text style={styles.feedName} numberOfLines={1}>{post.author_name}</Text></View>
            <View style={styles.feedMetaRow}>
              <Text style={styles.feedMeta}>{relativeTime(post.created_at)}</Text>
              {audienceMeta ? <View style={styles.audienceMeta}><Ionicons name={audienceMeta.icon as never} size={12} color={MUTED} /><Text style={styles.audienceMetaText}>{audienceMeta.label}</Text></View> : null}
            </View>
          </View>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  feedSectionHeader: { paddingHorizontal: 2, paddingTop: 2, gap: 1 }, feedSectionLabel: { color: TEXT, fontSize: 15, fontWeight: '900' }, feedSectionHint: { color: '#7F8B83', fontSize: 11.5, lineHeight: 16 }, feedCard: { backgroundColor: 'transparent', paddingHorizontal: 2, paddingVertical: 10, gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#26332C' }, feedHeader: { flexDirection: 'row', alignItems: 'center', gap: 9 }, authorProfileTarget: { flex: 1, minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 9, borderRadius: 10 }, authorProfileTargetPressed: { opacity: 0.65 }, feedAvatar: { width: 41, height: 41, borderRadius: 21, borderWidth: 1, borderColor: '#738078', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }, feedAvatarText: { color: TEXT, fontWeight: '900', fontSize: 11.5 }, authorCopy: { flex: 1, minWidth: 0 }, authorLine: { flexDirection: 'row', alignItems: 'center', gap: 5 }, feedName: { color: TEXT, fontSize: 15.5, fontWeight: '900', maxWidth: '88%' }, feedMetaRow: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 2 }, feedMeta: { color: '#8F9B93', fontSize: 11.5 }, audienceMeta: { flexDirection: 'row', alignItems: 'center', gap: 3 }, audienceMetaText: { color: '#8F9B93', fontSize: 10.5, fontWeight: '700' }, postTypeBadge: { backgroundColor: '#1D2B24', borderRadius: 99, paddingHorizontal: 8, paddingVertical: 5 }, postTypeBadgeText: { color: '#D6C28D', fontSize: 9, fontWeight: '900', letterSpacing: 0.4 }, postImage: { width: '100%', height: 230, borderRadius: 14, backgroundColor: '#101813' }, feedBody: { color: '#E0E5E1', fontSize: 13.5, lineHeight: 20 }, inlineToken: { color: GOLD, fontWeight: '800' }, engagementRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, engagementLeft: { flexDirection: 'row', alignItems: 'center', gap: 6 }, engagementText: { color: MUTED, fontSize: 12, marginRight: 5 },
