    ...
    <Pressable style={({ pressed }) => [styles.feedCard, pressed && styles.pressed]} onPress={() => router.push(`/community/${post.id}`)}>
      <View style={styles.feedHeader}>
        ...
        >
          ...
          <View style={styles.authorCopy}><View style={styles.authorLine}><Text style={styles.feedName} numberOfLines={1}>{post.author_name}</Text><Ionicons name={icon as never} size={13} color={MUTED} /></View><Text style={styles.feedMeta}>{relativeTime(post.created_at)}</Text></View>
        </Pressable>
