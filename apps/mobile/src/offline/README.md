...
Offline safety and field storage are scoped to the currently signed-in profile. Safety sessions, breadcrumbs, queued actions, downloaded event packs, locally saved roster sweep state, Host Field Mode snapshots, and Arrival Mode scans are cleared when the account signs out or when a different profile becomes the active owner of the local store.
...
Offline Ticketing + Arrival Mode builds on the authorized Host Field Mode roster. The host can scan a QR code, enter an opaque credential code, or find an attendee by name. Credential validation is performed against the roster already saved on that device, so a network connection is not required for the initial validation or local arrival record. Arrival Mode reports `Valid ticket`, `Already checked in`, `Wrong event`, or `Invalid ticket` rather than treating every scan as a successful check-in.

Arrival records use a client-generated scan ID, a local device ID, and the original scan time. They are added to the shared offline queue below safety actions. When service returns, `sync_adventure_arrival_scan` verifies the paid attendee and QR credential against server records, establishes one canonical event check-in, and marks later scans for the same attendee as duplicates. This lets two or more host devices operate independently while offline and reconcile duplicate arrivals later. Instant cross-device duplicate detection is impossible while those devices have no communication path.

The arrival audit table does not store raw credential codes. QR scans store a SHA-256 credential fingerprint on the server for reconciliation. The existing ticket credential remains the source of truth. The local authorized roster still contains the opaque credential needed to validate tickets without a connection.

Arrival Mode also exposes Arrived, Not arrived, Left early, and duplicate counts. `Left early` reuses the local roster sweep state so it remains available inside field operations. Camera permission is optional because manual ticket-code entry and attendee name lookup remain available if camera access is denied or a QR code cannot be scanned.
