...
export default function GuidedOnboardingExperience() {
  ...
  const Primary = ({ label = 'Continue', disabled = false, onPress = next }: { label?: string; disabled?: boolean; onPress?: () => void }) => (
    <Pressable style={[styles.primary, disabled && styles.disabled]} disabled={disabled} onPress={onPress}><Text style={styles.primaryText}>{label}</Text><Ionicons name="arrow-forward" size={17} color={BG} /></Pressable>
  );

  const QuestionSheet = ({ eyebrow, title, children, label = 'Continue' }: { eyebrow: string; title: string; children: React.ReactNode; label?: string }) => (
    <View style={styles.sheet}><View style={styles.sheetHandle} /><Text style={styles.sheetEyebrow}>{eyebrow}</Text><Text style={styles.sheetTitle}>{title}</Text>{children}<Primary label={label} disabled={!canContinue} /></View>
  );
