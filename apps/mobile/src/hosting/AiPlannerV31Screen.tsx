...
function sourceSummary(plan: V3PlanState, tenant: AiPlannerTenantContext) {
  return [
    plan.city && plan.state ? `${plan.city}, ${plan.state}` : plan.city || (plan.virtualEvent ? 'Virtual' : ''),
...
function optionSection(option: string, current: AiPlannerSection | null) {
  ...
  if (['find venues','recommend locations','search again'].includes(normalized)) return 'venue';
...
export default function AiPlannerV31Screen({ tenant }: { tenant: AiPlannerTenantContext }) {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: 'assistant', text: 'Tell me what you are planning. Give me everything you know in one message if you want. I will pull the useful details into the plan.' }]);
  ...
  useEffect(() => {
    ...
    void getOutingHostAccess().then((access) => {
      setCanCreate(access.approved);
      if (!access.approved) setAccessMessage('Your current role can plan this event, but it cannot create the Event Workspace.');
    }).catch((caught) => {
      setCanCreate(false);
  ...
  useEffect(() => {
    ...
    async function restore() {
      try {
        ...
        // Planning remains available when draft recovery is unavailable.
      }
    }
    void restore();
    return () => { active = false; };
  ...
  async function persistStructuredDraft(nextPlan: V3PlanState, nextTurn: AiPlannerV3Turn, nextHistory: PlanningDraftChange[]) {
    if (!tenant.organizationId || !isPlanningDraftReady(nextPlan)) return;
    setSaveState('saving');
    setSaveIssue('');
    try {
      ...
      setSaveState('error');
      setSaveIssue(caught instanceof Error ? caught.message : 'Changes are not saved yet.');
    }
  }
  ...
  async function send(text = input, meta: { section?: AiPlannerSection | null; action?: AiPlannerV2Action; venueCandidate?: VenueCandidate | null } = {}) {
    ...
    if (/^undo$/i.test(trimmed)) {
      ...
      void persistStructuredDraft(previous, reviewed, changeHistory);
      return;
    }
    ...
    setError('');
    try {
      const next = await runAiPlannerV31Turn({
        message: trimmed,
        plan,
      ...
      void persistStructuredDraft(next.plan, next, changes);
      const chatHistory = completeMessages.filter((item): item is { role: 'user' | 'assistant'; text: string } => item.role !== 'system');
      void persistAiPlannerTurn({ sessionId, plan: next.plan, turn: {
      ...
      }, history: chatHistory }).then(setSessionId).catch(() => undefined);
      if (next.command === 'create_workspace') void createWorkspace(next.plan, next.taskPacks);
    } catch (caught) {
  ...
  async function openSection(section: AiPlannerSection) {
    ...
    setLoading(true);
    try {
      const next = await runAiPlannerV31Turn({ message: '', plan, history: [], tenant, section, action: 'section' });
      setTurn(next);
      setMessages((current) => [...current, { role: 'system', text: `Working on ${PLANNER_SECTION_LABELS[section]}` }, { role: 'assistant', text: next.message }]);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Unable to open that planning section.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveVenue(candidate: VenueCandidate) {
    ...
    try {
      await saveVenueToShortlist(tenant.organizationId, candidate, { eventType: plan.category || null, attendance: plan.capacity || plan.attendanceRange || null, city: plan.city || null, state: plan.state || null });
  ...
  async function preferVenue(candidate: VenueCandidate) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: 'system', text: `${candidate.name} marked as preferred` }]);
    } catch (caught) {
  ...
  async function blockVenue(candidate: VenueCandidate) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: 'system', text: `${candidate.name} will not be recommended` }]);
  ...
  function directEdit(key: 'title' | 'city' | 'state' | 'capacity' | 'startsAt' | 'endsAt', value: string) {
    ...
    next.fieldStates = { ...(next.fieldStates ?? {}), [key]: { status: 'confirmed', value: key === 'capacity' ? next.capacity : next[key], updatedAt: new Date().toISOString() } };
  ...
  const saveLabel = saveState === 'saving' ? 'Saving…' : saveState === 'saved' ? 'Saved' : saveState === 'error' ? 'Not saved yet' : planningDraftId ? 'Saved' : 'New plan';
  ...
  return <SafeAreaView style={styles.safe}>
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
      ...
      {saveIssue ? <Text numberOfLines={1} style={styles.saveIssue}>Changes are safe in this session. I will retry saving on the next update.</Text> : null}
      {accessMessage ? <View style={styles.permissionBanner}><Text style={styles.permissionText}>{accessMessage}</Text></View> : null}
      {resumeDraftId ? <View style={styles.resumeBanner}><View style={styles.flex}><Text style={styles.resumeLabel}>SAVED PLAN</Text><Text style={styles.resumeTitle}>{resumeTitle}</Text></View><Pressable onPress={() => void resumeLatest()} style={styles.resumeButton}><Text style={styles.resumeButtonText}>Resume</Text></Pressable></View> : null}

      <Pressable style={styles.summaryCard} onPress={() => setSummaryOpen((value) => !value)}>
        <View style={styles.summaryTop}><Text style={styles.summaryTitle}>{workspaceProgress.ready ? '✓ Core setup complete' : `${workspaceProgress.complete}/${workspaceProgress.total} core details`}</Text><Text style={styles.summaryReadiness}>{currentTurn.readiness}%</Text></View>
        <Text style={styles.summaryLine}>{summary || 'Your planning draft starts as soon as you give me the event idea.'}</Text>
      ...
      <ScrollView horizontal style={styles.sectionScroller} showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sections}>
        {orderedSections.map((section) => {
          const status = currentTurn.sectionStatuses[section];
          const selected = activeSection === section;
          return <Pressable key={section} onPress={() => void openSection(section)} style={[styles.sectionChip, selected && styles.sectionSelected]}><Text style={[styles.sectionStatus, statusStyle(status)]}>{statusLabel(status)}</Text><Text style={[styles.sectionText, selected && styles.sectionTextSelected]}>{PLANNER_SECTION_LABELS[section]}</Text></Pressable>;
        })}
      </ScrollView>

      <ScrollView ref={(node) => { scrollRef.current = node; }} style={styles.conversation} contentContainerStyle={styles.conversationContent} keyboardShouldPersistTaps="handled" onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: true })}>
        ...
        {currentTurn.venueResults.length ? <View style={styles.venueList}>
          ...
          {currentTurn.venueResults.map((candidate) => <VenueCard key={candidate.id} candidate={candidate} onUse={() => void selectVenue(candidate)} onSave={() => void saveVenue(candidate)} onPrefer={() => void preferVenue(candidate)} onBlock={() => void blockVenue(candidate)} />)}
        ...
        {workspaceProgress.ready ? <View style={styles.workspaceCard}><Text style={styles.workspaceTitle}>Ready for an Event Workspace</Text><Text style={styles.workspaceBody}>Optional planning sections can stay open. This creates the operational workspace and does not publish the event.</Text><Pressable disabled={creating || canCreate === false} style={[styles.workspaceButton, (creating || canCreate === false) && styles.disabled]} onPress={() => void createWorkspace()}>{creating ? <ActivityIndicator color="#152018" /> : <Text style={styles.workspaceButtonText}>Create Event Workspace</Text>}</Pressable></View> : null}
      ...
      <View style={styles.composer}>
        <TextInput value={input} onChangeText={setInput} placeholder="Tell me what changed, ask a question, or keep planning…" placeholderTextColor="#667169" multiline style={styles.input} />
        <View style={styles.composerActions}><Pressable disabled={!undoStack.length || loading} onPress={() => void send('Undo')} style={[styles.undoButton, (!undoStack.length || loading) && styles.disabled]}><Text style={styles.undoText}>Undo</Text></Pressable><Pressable disabled={!input.trim() || loading} onPress={() => void send()} style={[styles.sendButton, (!input.trim() || loading) && styles.disabled]}>{loading ? <ActivityIndicator color="#152018" /> : <Text style={styles.sendText}>Send</Text>}</Pressable></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  headerMeta: { color: '#839088', fontSize: 9, marginTop: 2 },
  reviewButton: { minHeight: 34, borderRadius: 10, borderWidth: 1, borderColor: '#665522', justifyContent: 'center', paddingHorizontal: 11 },
  reviewButtonText: { color: '#E2C567', fontSize: 10, fontWeight: '900' },
  statusBar: { minHeight: 32, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 9 },
  stage: { color: '#D7B45A', fontSize: 9, fontWeight: '900' },
  save: { color: '#7FA68A', fontSize: 9, fontWeight: '800' },
  saveError: { color: '#D99C5B' },
  privacy: { color: '#748078', fontSize: 9, marginLeft: 'auto' },
  saveIssue: { color: '#A98D62', fontSize: 8, paddingHorizontal: 16, paddingBottom: 5 },
  permissionBanner: { marginHorizontal: 14, marginBottom: 6, borderRadius: 10, borderWidth: 1, borderColor: '#624B2B', backgroundColor: '#211A10', padding: 9 },
  permissionText: { color: '#C8A777', fontSize: 9, lineHeight: 14 },
  resumeBanner: { marginHorizontal: 14, marginBottom: 6, borderRadius: 12, borderWidth: 1, borderColor: '#3A493F', backgroundColor: '#111914', padding: 9, flexDirection: 'row', alignItems: 'center', gap: 10 },
  ...
  summaryCard: { marginHorizontal: 14, borderRadius: 12, borderWidth: 1, borderColor: '#314037', backgroundColor: '#121914', padding: 10 },
  ...
  summaryExpanded: { marginTop: 7, borderTopWidth: 1, borderTopColor: '#253129', paddingTop: 7, gap: 4 },
  summaryDetail: { color: '#9CA89F', fontSize: 9, lineHeight: 13 },
  sectionScroller: { flexGrow: 0, flexShrink: 0, maxHeight: 54, minHeight: 48 },
  sections: { minHeight: 48, alignItems: 'center', paddingHorizontal: 14, paddingVertical: 7, gap: 7 },
  sectionChip: { height: 34, flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 18, borderWidth: 1, borderColor: '#303C35', backgroundColor: '#111813', paddingHorizontal: 10, flexShrink: 0 },
  ...
  conversationContent: { paddingHorizontal: 14, paddingTop: 4, paddingBottom: 18, gap: 9 },
  ...
  option: { minHeight: 35, borderRadius: 18, borderWidth: 1, borderColor: '#3B4840', backgroundColor: '#111813', justifyContent: 'center', paddingHorizontal: 11 },
  ...
  input: { minHeight: 48, maxHeight: 100, borderRadius: 13, borderWidth: 1, borderColor: '#334038', backgroundColor: '#121914', color: '#FFF8E8', fontSize: 11, lineHeight: 16, paddingHorizontal: 11, paddingVertical: 9, textAlignVertical: 'top' },
  composerActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 },
  undoButton: { minHeight: 32, justifyContent: 'center', paddingHorizontal: 8 },
  undoText: { color: '#87948B', fontSize: 9, fontWeight: '800' },
  sendButton: { minWidth: 72, minHeight: 34, borderRadius: 10, backgroundColor: '#D7B45A', alignItems: 'center', justifyContent: 'center' },
