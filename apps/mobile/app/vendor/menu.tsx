...
export default function VendorMenuScreen() {
  ...
  useEffect(() => {
    ...
    void getOutingHostAccess().then((access) => {
      if (active) setHostApproved(access.approved);
    }).catch(() => {
      if (active) setHostApproved(false);
    });
    return () => { active = false; };
  }, []);
  ...
  return <SafeAreaView style={styles.safe}>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {hostApproved ? <Pressable style={styles.workspaceRow} onPress={() => router.replace('/host' as never)}><Text style={styles.workspaceTitle}>Host Center</Text><Text style={styles.chevron}>›</Text></Pressable> : null}
      <Pressable style={[styles.workspaceRow, hostApproved && styles.workspaceSpacing]} onPress={() => router.replace('/(tabs)' as never)}><Text style={styles.workspaceTitle}>Member App</Text><Text style={styles.chevron}>›</Text></Pressable>
    </ScrollView>
  </SafeAreaView>;
}
