...
export default function ProfileScreen(){
 ...
 async function chooseProfilePhoto(){
  ...
  setPhotoBusy(true);
  try{const asset=result.assets[0];if(!asset.base64)throw new Error('That photo could not be prepared safely.');const avatarUrl=await uploadProfilePhoto({uri:asset.uri,base64:asset.base64,mimeType:asset.mimeType});setData((current:any)=>({...current,profile:{...current.profile,avatar_url:avatarUrl}}));setMessage('Profile photo updated.')}
 ...
 async function chooseCover(){
  ...
  const result=await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'],allowsEditing:true,aspect:[16,9],base64:true,quality:.85});
  if(result.canceled||!result.assets?.[0])return;
  setCoverBusy(true);
  try{const asset=result.assets[0];if(!asset.base64)throw new Error('That cover could not be prepared safely.');const coverUrl=await uploadProfileCover({uri:asset.uri,base64:asset.base64,mimeType:asset.mimeType});setData((current:any)=>({...current,profile:{...current.profile,cover_url:coverUrl}}));setMessage('Cover image updated.')}
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
  ...
  <View style={styles.rankCard}>
   <View style={styles.rankIcon}><RankEmblem rank={rank} size={58}/></View>
  ...
  <View style={styles.journeySummary}><View style={styles.journeySummaryHeading}><AppIcon name="adventure" color="#F5C341" size={17}/><Text style={styles.journeySummaryTitle}>YOUR JOURNEY</Text></View><View style={styles.journeyStatsRow}><JourneyStat value={stamps.length} label="Stamps" onPress={()=>router.push('/member/stamps')}/><View style={styles.statDivider}/><JourneyStat value={badges.length} label="Badges" onPress={()=>router.push('/member/badges')}/></View></View>
...
const styles=StyleSheet.create({
 safe:{flex:1,backgroundColor:'#09110F'},center:{flex:1,backgroundColor:'#09110F',alignItems:'center',justifyContent:'center'},content:{paddingHorizontal:18,paddingTop:8,paddingBottom:72,gap:14},editContent:{padding:20,paddingBottom:160,gap:14},pressed:{opacity:.58},
 ...
 rankCard:{backgroundColor:'#111A17',borderWidth:1,borderColor:'#32443A',borderRadius:20,padding:14,flexDirection:'row',alignItems:'center',gap:13},rankIcon:{width:68,alignItems:'center',justifyContent:'center'},rankCopy:{flex:1,minWidth:0},rankName:{color:'#F7F8F3',fontSize:17,fontWeight:'900',letterSpacing:.5},rankSubtitle:{color:'#67CFC8',fontSize:11.5,fontWeight:'800',marginTop:2},rankAdventureCount:{color:'#D4DBD7',fontSize:13,fontWeight:'800',marginTop:5},progressTrack:{height:5,borderRadius:99,backgroundColor:'#194B4B',overflow:'hidden',marginTop:8},progressFill:{height:'100%',backgroundColor:'#F5C341',borderRadius:99},
 journeySummary:{backgroundColor:'#101916',borderRadius:18,borderWidth:1,borderColor:'#29372F',padding:13},journeySummaryHeading:{flexDirection:'row',alignItems:'center',gap:7,paddingBottom:10,borderBottomWidth:1,borderBottomColor:'#26332D'},journeySummaryTitle:{color:'#F5C341',fontSize:12,fontWeight:'900',letterSpacing:.8},journeyStatsRow:{flexDirection:'row',alignItems:'stretch',paddingTop:9},journeyStat:{flex:1,alignItems:'center',justifyContent:'center',paddingVertical:2},journeyStatValue:{color:'#F7F8F3',fontSize:23,fontWeight:'900'},journeyStatLabel:{color:'#AAB5B0',fontSize:11.5,fontWeight:'700',marginTop:1},statDivider:{width:1,backgroundColor:'#324038',marginVertical:4},
