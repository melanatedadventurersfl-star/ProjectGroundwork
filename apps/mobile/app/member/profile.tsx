...
export default function ProfileScreen(){
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
  ...
  {tab==='photos'?<View style={styles.photosSection}>
   <View><Text style={styles.sectionTitle}>Photos</Text><Text style={styles.photosIntro}>Moments you’ve saved from your adventures.</Text></View>
   {albums.map(album=><Pressable key={album.adventure_id} style={({pressed})=>[styles.photoAlbumRow,pressed&&styles.pressed]} onPress={()=>router.push(`/passport/photos/${album.adventure_id}`)}>
    {album.cover_url?<Image source={{uri:album.cover_url}} style={styles.albumThumb}/>:<View style={styles.albumThumbPlaceholder}><AppIcon name="photos" color="#D7B45A" size={23}/></View>}
    <View style={styles.albumCopy}><Text style={styles.albumTitle} numberOfLines={2}>{album.title}</Text><Text style={styles.albumMeta}>{album.memories.length} photo{album.memories.length===1?'':'s'}</Text></View>
...
const styles=StyleSheet.create({
 ...
 photosSection:{gap:10},photosIntro:{color:'#8F9C95',fontSize:12.5,marginTop:2},photoAlbumRow:{minHeight:70,flexDirection:'row',alignItems:'center',gap:11,paddingVertical:8,borderBottomWidth:1,borderBottomColor:'#26332D'},albumThumb:{width:58,height:58,borderRadius:12,backgroundColor:'#17211C'},albumThumbPlaceholder:{width:58,height:58,borderRadius:12,backgroundColor:'#17241E',alignItems:'center',justifyContent:'center'},albumCopy:{flex:1,minWidth:0},albumTitle:{color:'#F7F8F3',fontSize:14,fontWeight:'900',lineHeight:18},albumMeta:{color:'#8F9B94',fontSize:11.5,marginTop:3},photoEmpty:{alignItems:'center',gap:6,paddingVertical:24,paddingHorizontal:16},
