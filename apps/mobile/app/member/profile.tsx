...
import { getJourney, getMemberBadges, getMemoryAlbums, getPassportStamps, type MemberBadge, type MemoryAlbum, type PassportStamp } from '../../src/passport/api';
...
type EarnedStampCard = {
 stamp: PassportStamp;
 art: StampCatalogItem | null;
};
...
function FeaturedStamp({item}:{item:EarnedStampCard}){
 return <Pressable style={({pressed})=>[styles.stampCard,pressed&&styles.pressed]} onPress={()=>router.push('/member/stamps')}>
  {item.art?<Image source={item.art.source} style={styles.stampImage} resizeMode="contain"/>:<View style={styles.genericStamp}><AppIcon name="adventure" color="#F5C341" size={34}/></View>}
  <Text style={styles.stampTitle} numberOfLines={2}>{item.stamp.title}</Text>
 </Pressable>;
}
...
export default function ProfileScreen(){
 ...
 async function load(){
  setLoading(true);
  try{
   const [base,nextJourney,nextStamps,nextBadges,nextAlbums]=await Promise.all([getMemberBasecamp(),getJourney(),getPassportStamps(),getMemberBadges(),getMemoryAlbums()]);
   setData(base);setJourney(nextJourney);setStamps(nextStamps);setBadges(nextBadges);setAlbums(nextAlbums);
 ...
 const earnedStampCards=useMemo<EarnedStampCard[]>(()=>stamps.slice(0,6).map(stamp=>({stamp,art:STAMP_CATALOG.find(item=>(stamp.code&&item.code===stamp.code)||item.title.toLowerCase()===stamp.title.toLowerCase())??null})),[stamps]);
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
  <View style={styles.coverShell}>
   ...
   <Pressable onPress={coverMenu} accessibilityRole="button" accessibilityLabel="Change cover image" style={styles.coverEditButton}><AppIcon name="camera" color="#F7F8F3" size={14}/></Pressable>
   <Pressable accessibilityRole="button" accessibilityLabel="Edit profile" style={({pressed})=>[styles.profileEditButton,pressed&&styles.pressed]} onPress={()=>setEditing(true)}><AppIcon name="edit" color="#F7F8F3" size={14}/><Text style={styles.profileEditText}>Edit</Text></Pressable>
  </View>
  ...
  {Array.isArray(profile.interests)&&profile.interests.length?<View style={styles.headerChips}>{profile.interests.map((interest:string)=><Text key={interest} style={styles.headerChip}>{interest}</Text>)}</View>:null}
  <Text style={styles.joinedText}>Joined {profile.created_at?new Date(profile.created_at).toLocaleDateString(undefined,{month:'long',year:'numeric'}):'recently'}</Text>

  <View style={styles.tabs}>{(['journey','posts','photos'] as ProfileTab[]).map(value=><Pressable key={value} onPress={()=>setTab(value)} style={styles.tab}><Text style={[styles.tabText,tab===value&&styles.tabTextActive]}>{value.charAt(0).toUpperCase()+value.slice(1)}</Text>{tab===value?<View style={styles.tabUnderline}/>:null}</Pressable>)}</View>
  ...
  {tab==='journey'?<>
   ...
   {earnedStampCards.length?<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stampsRow}>{earnedStampCards.map(item=><FeaturedStamp key={item.stamp.stamp_id} item={item}/>)}</ScrollView>:<View style={styles.empty}><Text style={styles.emptyTitle}>No stamps earned yet</Text><Text style={styles.muted}>Official Adventure stamps will appear here after you earn them.</Text></View>}
  </>:null}

  {tab==='posts'?<ProfilePosts/>:null}
  {tab==='photos'?<View style={styles.photosSection}>
   <View><Text style={styles.sectionTitle}>Photos</Text><Text style={styles.photosIntro}>Your adventure albums. Tap one to open the gallery.</Text></View>
   {albums.map(album=><Pressable key={album.adventure_id} style={({pressed})=>[styles.photoAlbumCard,pressed&&styles.pressed]} onPress={()=>router.push(`/passport/photos/${album.adventure_id}`)}>
    <View style={styles.albumCoverWrap}>
     {album.cover_url?<Image source={{uri:album.cover_url}} style={styles.albumCover}/>:<View style={styles.albumCoverPlaceholder}><AppIcon name="photos" color="#D7B45A" size={34}/></View>}
     <View style={styles.albumCoverShade}/>
     <View style={styles.albumCountPill}><AppIcon name="photos" color="#FFF8E8" size={12}/><Text style={styles.albumCountText}>{album.memories.length}</Text></View>
    </View>
    <View style={styles.albumFooter}>
     <View style={styles.albumCopy}><Text style={styles.albumTitle} numberOfLines={2}>{album.title}</Text><Text style={styles.albumMeta}>Open gallery</Text></View>
     <View style={styles.albumArrow}><AppIcon name="chevron-forward" color="#D7B45A" size={18}/></View>
    </View>
   </Pressable>)}
   {!albums.length?<View style={styles.photoEmpty}><AppIcon name="photos" color="#D7B45A" size={28}/><Text style={styles.emptyTitle}>No adventure photos yet</Text><Text style={styles.muted}>Photos you add to memories will collect here automatically.</Text></View>:null}
  </View>:null}
...
const styles=StyleSheet.create({
 ...
 coverShell:{aspectRatio:16/9,borderRadius:22,overflow:'hidden',borderWidth:1,borderColor:'#27332F',backgroundColor:'#111A17',position:'relative'},coverImage:{width:'100%',height:'100%',resizeMode:'cover'},coverShade:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(4,10,8,.08)'},coverPlaceholder:{flex:1,alignItems:'center',justifyContent:'center',gap:7,backgroundColor:'#122019'},coverPlaceholderText:{color:'#D7B45A',fontWeight:'800'},coverEditButton:{position:'absolute',right:10,top:10,width:32,height:32,borderRadius:16,backgroundColor:'rgba(9,17,15,.82)',borderWidth:1,borderColor:'rgba(245,195,65,.55)',alignItems:'center',justifyContent:'center'},profileEditButton:{position:'absolute',right:10,bottom:10,minHeight:36,borderRadius:18,paddingHorizontal:12,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:6,backgroundColor:'rgba(9,17,15,.86)',borderWidth:1,borderColor:'rgba(247,248,243,.28)'},profileEditText:{color:'#F7F8F3',fontSize:12,fontWeight:'900'},
 profileIdentity:{flexDirection:'row',alignItems:'flex-start',gap:11,paddingHorizontal:10,zIndex:2},avatarWrap:{width:76,height:76,position:'relative',marginTop:-38,borderRadius:38,borderWidth:3,borderColor:'#09110F',backgroundColor:'#09110F'},mainCameraBadge:{position:'absolute',right:-2,bottom:2,width:23,height:23,borderRadius:12,backgroundColor:'#F5C341',borderWidth:2,borderColor:'#09110F',alignItems:'center',justifyContent:'center'},identityCopy:{flex:1,minWidth:0,paddingTop:2,paddingBottom:1},name:{fontSize:25,fontWeight:'900',lineHeight:28,color:'#F7F8F3',letterSpacing:-.35},handle:{color:'#F5C341',fontSize:13,fontWeight:'800',marginTop:1},locationRow:{flexDirection:'row',alignItems:'center',gap:4,marginTop:2},location:{color:'#AEB9B4',fontSize:13},rankLine:{flexDirection:'row',alignItems:'center',gap:6,marginTop:4},rankLineText:{color:'#F7F8F3',fontSize:11.5,fontWeight:'900',letterSpacing:.5},rankLineMeta:{color:'#67CFC8',fontSize:10.5,fontWeight:'800'},
 statLine:{flexDirection:'row',alignItems:'center',paddingHorizontal:10,gap:9},statLink:{flexDirection:'row',alignItems:'baseline',gap:4},statValue:{color:'#F5C341',fontSize:13,fontWeight:'900'},statLabel:{color:'#B9C3BE',fontSize:11,fontWeight:'800'},statDot:{color:'#52605A',fontSize:12},bioText:{color:'#D4DBD7',fontSize:14.5,lineHeight:20,paddingHorizontal:10,marginTop:1},headerChips:{flexDirection:'row',flexWrap:'wrap',gap:6,paddingHorizontal:10},headerChip:{color:'#E5C977',backgroundColor:'#203029',borderRadius:999,paddingHorizontal:9,paddingVertical:5,fontSize:10.5,fontWeight:'800'},joinedText:{color:'#819088',fontSize:11.5,paddingHorizontal:10,marginTop:-2},
 tabs:{flexDirection:'row',borderBottomWidth:1,borderBottomColor:'#28322E'},tab:{flex:1,alignItems:'center',paddingTop:8,paddingBottom:8,position:'relative'},tabText:{color:'#A8B2AD',fontSize:13,fontWeight:'800'},tabTextActive:{color:'#F5C341'},tabUnderline:{height:2,backgroundColor:'#F5C341',position:'absolute',bottom:-1,left:28,right:28,borderRadius:4},
 sectionHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginTop:2},sectionTitle:{color:'#F7F8F3',fontSize:20,fontWeight:'900'},sectionLinkWrap:{flexDirection:'row',alignItems:'center',gap:2},sectionLink:{color:'#67CFC8',fontSize:12.5,fontWeight:'800'},stampsRow:{gap:9,paddingRight:20},stampCard:{width:122,minHeight:150,backgroundColor:'#111A17',borderRadius:17,borderWidth:1,borderColor:'#29342F',padding:8,alignItems:'center'},stampImage:{width:'100%',height:112},stampTitle:{color:'#F7F8F3',fontWeight:'800',fontSize:10.5,lineHeight:13,textAlign:'center',marginTop:2},genericStamp:{width:'100%',height:112,borderRadius:14,backgroundColor:'#18251F',alignItems:'center',justifyContent:'center'},badgesRow:{gap:9,paddingRight:20},badgeCard:{width:116,minHeight:128,backgroundColor:'#111A17',borderRadius:17,borderWidth:1,borderColor:'#29342F',padding:10,alignItems:'center',justifyContent:'center'},badgeTitle:{color:'#F7F8F3',fontWeight:'800',fontSize:10.5,lineHeight:13,textAlign:'center',marginTop:6},genericBadge:{width:64,height:64,borderRadius:32,borderWidth:1,borderColor:'#D7B45A',backgroundColor:'#21302A',alignItems:'center',justifyContent:'center'},
 empty:{backgroundColor:'#111A17',borderRadius:18,borderWidth:1,borderColor:'#28362E',padding:16},emptyTitle:{color:'#F7F8F3',fontWeight:'900'},muted:{color:'#96A39B',lineHeight:20},
 photosSection:{gap:12},photosIntro:{color:'#8F9C95',fontSize:12.5,marginTop:2},photoAlbumCard:{borderRadius:20,overflow:'hidden',backgroundColor:'#111A17',borderWidth:1,borderColor:'#28362E'},albumCoverWrap:{width:'100%',aspectRatio:16/9,position:'relative',backgroundColor:'#17211C'},albumCover:{width:'100%',height:'100%'},albumCoverPlaceholder:{width:'100%',height:'100%',alignItems:'center',justifyContent:'center',backgroundColor:'#17241E'},albumCoverShade:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(4,10,8,.08)'},albumCountPill:{position:'absolute',right:10,top:10,minHeight:28,borderRadius:14,paddingHorizontal:9,backgroundColor:'rgba(9,17,15,.82)',flexDirection:'row',alignItems:'center',gap:5},albumCountText:{color:'#FFF8E8',fontSize:11,fontWeight:'900'},albumFooter:{minHeight:62,paddingHorizontal:14,paddingVertical:10,flexDirection:'row',alignItems:'center',gap:10},albumCopy:{flex:1,minWidth:0},albumTitle:{color:'#F7F8F3',fontSize:15,fontWeight:'900',lineHeight:19},albumMeta:{color:'#8F9B94',fontSize:11.5,marginTop:3},albumArrow:{width:32,height:32,borderRadius:16,backgroundColor:'#1B2922',alignItems:'center',justifyContent:'center'},photoEmpty:{alignItems:'center',gap:6,paddingVertical:24,paddingHorizontal:16},
 card:{backgroundColor:'#111A17',borderRadius:18,borderWidth:1,borderColor:'#28362E',padding:16,gap:10},cardTitle:{color:'#F7F8F3',fontSize:20,fontWeight:'900'},
 editTopBar:{minHeight:54,flexDirection:'row',alignItems:'center',justifyContent:'space-between',paddingHorizontal:20,borderBottomWidth:1,borderBottomColor:'#24302B',backgroundColor:'#09110F'},editBack:{flexDirection:'row',alignItems:'center',alignSelf:'flex-start'},editBackText:{color:'#F5C341',fontWeight:'800'},saveTopButton:{minWidth:72,minHeight:36,borderRadius:999,paddingHorizontal:16,alignItems:'center',justifyContent:'center',backgroundColor:'#D7B45A'},saveTopButtonText:{color:'#17211C',fontSize:13,fontWeight:'900'},coverEditor:{backgroundColor:'#111A17',borderRadius:20,borderWidth:1,borderColor:'#28362E',overflow:'hidden'},coverEditorPreview:{aspectRatio:16/9,backgroundColor:'#0E1713'},mediaEditCopy:{padding:14,gap:4},photoEditor:{flexDirection:'row',alignItems:'center',gap:14,backgroundColor:'#111A17',borderRadius:20,borderWidth:1,borderColor:'#28362E',padding:15},photoPressable:{width:84,height:84,borderRadius:42,position:'relative'},cameraBadge:{position:'absolute',right:-1,bottom:2,width:28,height:28,borderRadius:14,backgroundColor:'#F5C341',borderWidth:3,borderColor:'#111A17',alignItems:'center',justifyContent:'center'},photoBusy:{position:'absolute',left:0,right:0,top:0,bottom:0,borderRadius:42,backgroundColor:'rgba(9,17,15,.68)',alignItems:'center',justifyContent:'center'},mediaBusy:{...StyleSheet.absoluteFillObject,backgroundColor:'rgba(9,17,15,.68)',alignItems:'center',justifyContent:'center'},editSectionTitle:{fontSize:19,fontWeight:'900',color:'#FFF8E8'},photoAction:{color:'#F5C341',fontWeight:'900',marginTop:2},
