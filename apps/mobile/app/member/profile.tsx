   ...
   <View style={styles.identityRankRow}>
    <View style={styles.identityCluster}>
     <Pressable onPress={photoMenu} style={styles.avatarWrap}>
      <Avatar url={profile.avatar_url} name={profile.display_name} size={68}/>
      <View style={styles.mainCameraBadge}><AppIcon name="camera" color="#121A17" size={12}/></View>
     </Pressable>
     <View style={styles.heroCopy}>
      <Text style={styles.name} numberOfLines={2} ellipsizeMode="tail">{profile.display_name??'Adventurer'}</Text>
      {profile.username?<Text style={styles.handle} numberOfLines={1}>@{profile.username}</Text>:null}
      {profile.city_visible!==false&&location?<View style={styles.locationRow}><AppIcon name="location" color="#AEB9B4" size={14}/><Text style={styles.location} numberOfLines={1}>{location}</Text></View>:null}
     </View>
    </View>
    <View style={styles.rankIdentity}>
     <RankEmblem rank={rank} size={44}/>
     <Text style={styles.rankCompact} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={.82}>{rank.toUpperCase()}</Text>
