...
function FavoriteStamp({stamp}:{stamp:PassportStamp}){
...
export default function ProfileScreen(){
 ...
 const currentMinimum=rankLadder.find(([name])=>name===rank)?.[1]??0;
 const nextMinimum=nextRank?.[1]??currentMinimum;
 const progress=nextMinimum===currentMinimum?1:Math.max(0,Math.min(1,(journey.length-currentMinimum)/(nextMinimum-currentMinimum)));
 ...
 const favorites=stamps.slice(0,3);
 ...
 return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
  ...
  {tab==='journey'?<>
   <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>My Favorites</Text>{stamps.length>3?<Pressable onPress={()=>setTab('photos')}><Text style={styles.sectionLink}>View all  ›</Text></Pressable>:null}</View>
   {favorites.length?<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.favoritesRow}>{favorites.map(stamp=><FavoriteStamp key={stamp.stamp_id} stamp={stamp}/>)}</ScrollView>:<View style={styles.empty}><Text style={styles.emptyTitle}>Your favorite adventures will live here</Text><Text style={styles.muted}>Earn stamps as you complete official adventures.</Text></View>}
...
const styles=StyleSheet.create({
 ...
 sectionHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginTop:1},sectionTitle:{color:'#F7F8F3',fontSize:21,fontWeight:'900'},sectionLink:{color:'#65C9C4',fontSize:13,fontWeight:'700'},favoritesRow:{gap:10,paddingRight:8},favoriteCard:{width:128,minHeight:156,backgroundColor:'#111A17',borderRadius:18,borderWidth:1,borderColor:'#29342F',padding:11,alignItems:'center'},favoriteArt:{height:78,alignItems:'center',justifyContent:'center'},favoriteTitle:{color:'#F7F8F3',fontWeight:'900',fontSize:13,textAlign:'center',marginTop:4},favoriteMeta:{color:'#60C9C3',fontSize:11,marginTop:4},genericStamp:{width:64,height:64,borderRadius:32,borderWidth:2,borderColor:'#D7B45A',backgroundColor:'#21302A',alignItems:'center',justifyContent:'center'},genericStampIcon:{color:'#F5C341',fontSize:25},
 timeline:{gap:0},timelineRow:{flexDirection:'row'},timelineRail:{width:45,alignItems:'center'},completeDot:{width:32,height:32,borderRadius:16,backgroundColor:'#1D7B48',alignItems:'center',justifyContent:'center',zIndex:2},completeCheck:{color:'#F7F8F3',fontWeight:'900',fontSize:17},timelineLine:{width:2,backgroundColor:'#39473F',flex:1,minHeight:94},journeyCard:{flex:1,minHeight:116,backgroundColor:'#111A17',borderRadius:18,borderWidth:1,borderColor:'#28332F',marginBottom:12,padding:13,flexDirection:'row',alignItems:'center',gap:11},journeyBadge:{width:48,height:48,borderRadius:12,backgroundColor:'#16302A',alignItems:'center',justifyContent:'center'},journeyBadgeIcon:{color:'#F5C341',fontSize:25,fontWeight:'900'},journeyCopy:{flex:1},journeyTitle:{color:'#F7F8F3',fontSize:16,fontWeight:'900'},journeyMeta:{color:'#AAB6B0',fontSize:12.5,marginTop:4},journeyTags:{flexDirection:'row',flexWrap:'wrap',gap:6,marginTop:10},completeTag:{backgroundColor:'#17492C',color:'#B9E8C8',borderRadius:999,paddingHorizontal:9,paddingVertical:5,fontSize:10.5,fontWeight:'800'},stampTag:{borderWidth:1,borderColor:'#287E79',color:'#62CDC7',borderRadius:999,paddingHorizontal:9,paddingVertical:4,fontSize:10.5,fontWeight:'800'},
