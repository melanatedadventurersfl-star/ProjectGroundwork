  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Pressable style={styles.card} onPress={() => router.push('/member/support' as never)}><Text style={styles.cardTitle}>Support</Text><Text style={styles.detail}>{data?.support?.filter((item: any) => !['resolved','closed'].includes(item.status)).length ?? 0} open request(s)</Text><Text style={styles.link}>Contact support & track requests →</Text></Pressable>
