...
function matchesQuickTags(item: { title: string; description?: string | null; category?: string | null; starts_at: string }, tags: string[]) {
  ...
  return tags.every((tag) => {
    if (tag === 'Upcoming') return new Date(item.starts_at).getTime() >= Date.now();
    if (tag === 'Weekend') return isWeekend(item.starts_at);
    return inferred.includes(tag);
  });
}
...
function LocalCard({ event, distance }: { event: LocalEvent; distance?: number | null }) {
  ...
  return (
    <Pressable style={s.card} onPress={() => router.push({ pathname: '/local-events/[id]', params: { id: event.id } })}>
      <View style={s.cardTopRow}><Text style={s.badge}>LOCAL EVENT</Text>{distance != null ? <Text style={s.distance}>{Math.round(distance)} mi</Text> : null}</View>
...
export default function ExploreScreen() {
  ...
  const localWithDistance = useMemo(() => events.map((event) => {
    ...
    const locationTerms = ['florida', 'fl', event.city.toLowerCase(), event.state.toLowerCase()];
    const textMatch = !q || searchable.includes(q) || (searchCenter != null && locationTerms.some((term) => q.includes(term)));
  ...
  const sortedLocal = useMemo(() => [...localWithDistance].sort((a, b) => {
    ...
    if (sort === 'newest') return b.event.id.localeCompare(a.event.id);
    return new Date(a.event.starts_at).getTime() - new Date(b.event.starts_at).getTime();
  }), [localWithDistance, sort]);

  const filteredAdventures = useMemo(() => adventures.filter((item) => matchesQuickTags(item, selectedTags)).sort((a, b) => {
    if (sort === 'newest') return b.id.localeCompare(a.id);
    if (sort === 'price') return 0;
    return new Date(a.starts_at).getTime() - new Date(b.starts_at).getTime();
  }), [adventures, selectedTags, sort]);
