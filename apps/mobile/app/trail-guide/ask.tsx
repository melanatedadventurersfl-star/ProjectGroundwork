...
type PlannerPhase = 'discover' | 'build' | 'refine' | 'confirm' | 'go';
...
function planLabel(result: MemberGuideResult | null) {
  ...
  return count > 0 ? `${count} stop${count === 1 ? '' : 's'}` : 'No stops yet';
}
...
function shortFollowUp(label: string) {
  ...
  if (lower.includes('second')) return 'Swap stop';
  return label;
}
...
export default function AskGoScreen() {
  ...
  function handleFollowUp(follow: string) {
    if (isFoodFollowUp(follow)) return openFoodPicker();
    void ask(follow);
  }
    ...
    return (
      <View style={styles.planCard}>
        <View style={styles.planTop}>
          ...
          {!confirmed && exchanges.length > 1 ? (
            <Pressable onPress={() => void ask('Undo that and bring back my previous plan')} style={styles.smallButton}>
        ...
        {!confirmed ? (
          <View style={styles.actionWrap}>
            ...
            {canSwap ? (
              <Pressable style={styles.actionChip} onPress={() => void ask('Swap the second stop in this plan')}><Text style={styles.actionText}>Swap stop</Text></Pressable>
            ) : (
              <Pressable style={styles.actionChip} onPress={() => void ask('Add another outdoor stop that fits this plan and the time I have')}><Text style={styles.actionText}>+ Stop</Text></Pressable>
            )}
            {canShorten ? <Pressable style={styles.actionChip} onPress={() => void ask('Make this plan shorter')}><Text style={styles.actionText}>Shorter</Text></Pressable> : null}
        ...
        {!confirmed ? (
          <Pressable style={styles.confirmButton} onPress={() => setConfirmed(true)}>
            <Text style={styles.confirmText}>Looks good · Finish plan</Text>
          </Pressable>
        ) : null}
      </View>
    );
    ...
    if (foodStage === 'kind') {
      return (
        <View style={styles.inlineCard}>
          <Text style={styles.inlineTitle}>What kind of food stop?</Text>
          <Text style={styles.inlineText}>I’ll show choices first. Nothing gets added until you pick one.</Text>
          <View style={styles.chipWrap}>
            {FOOD_KINDS.map((kind) => <Pressable key={kind} style={styles.choiceChip} onPress={() => void findFood(kind)}><Text style={styles.choiceText}>{kind}</Text></Pressable>)}
          </View>
        </View>
      );
    }
    if (foodStage === 'loading') {
      return <View style={styles.loadingRow}><ActivityIndicator color="#D7B45A" /><Text style={styles.loadingText}>Finding food options in {cityName}…</Text></View>;
    }
    if (foodStage === 'results') {
      return (
        <View style={styles.inlineCard}>
          <Text style={styles.inlineTitle}>{foodOptions.length ? 'Pick a food stop' : 'No matching food stops loaded yet'}</Text>
          <Text style={styles.inlineText}>{foodOptions.length ? 'Choose one and I’ll add it to the current plan.' : 'I won’t substitute parks or outdoor places for restaurants. Try another food style or keep the plan as-is.'}</Text>
          {foodOptions.map((option) => (
            <Pressable key={option.id} style={styles.foodRow} onPress={() => addFood(option)}>
              <View style={styles.foodCopy}>
                <Text style={styles.foodName}>{option.name}</Text>
                <Text style={styles.foodMeta}>{option.category || 'Dining'}{option.address ? ` · ${option.address}` : ''}</Text>
                {option.description ? <Text style={styles.foodDescription} numberOfLines={2}>{option.description}</Text> : null}
                {option.verified && option.ownershipTags.length > 0 ? <Text style={styles.verified}>✓ Verified community-owned</Text> : null}
              </View>
              <Text style={styles.addText}>Add</Text>
            </Pressable>
          ))}
          <Pressable onPress={() => setFoodStage('kind')}><Text style={styles.tryAgain}>Try another style</Text></Pressable>
        </View>
      );
    }
    return null;
    ...
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <View style={styles.headerIdentity}>
          ...
          <View><Text style={styles.headerTitle}>Ask Go</Text><Text style={styles.headerSubtitle}>{phase === 'go' ? 'Your day is ready' : planning ? 'Planning with you' : 'Your outdoor guide'}</Text></View>
        </View>
      ...
      <ScrollView ref={scrollRef} style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        {exchanges.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.intro}>Tell me the vibe and time you have, or tap Plan my day and I’ll guide the rest.</Text>
            <Pressable style={styles.planMyDay} onPress={startPlanMyDay}>
              <View><Text style={styles.planMyDayTitle}>✦  Plan my day</Text><Text style={styles.planMyDayText}>A complete adventure in a few taps</Text></View>
              <AppIcon name="chevron-forward" color="#172017" size={19} />
            </Pressable>
            <View style={styles.quickStack}>{QUICK_PROMPTS.slice(1).map((prompt) => <Pressable key={prompt} style={styles.quickCard} onPress={() => void ask(prompt)}><Text style={styles.quickText}>{prompt}</Text></Pressable>)}</View>
          </View>
        ) : null}
        ...
        {exchanges.map((exchange, index) => {
          ...
          return (
            <View key={exchange.id} style={styles.exchange}>
              ...
              {exchange.loading ? <View style={styles.goLine}><ActivityIndicator color="#D7B45A" size="small" /></View> : null}
              {exchange.error ? <View style={styles.goLine}><Text style={styles.errorText}>{exchange.error}</Text></View> : null}
              {exchange.result ? <View style={styles.goLine}><Text style={styles.goText}>{exchange.result.answer}</Text></View> : null}
              {isLatest && exchange.result?.dayPlan.length ? renderPlan(exchange.result) : null}
              {isLatest && exchange.result && !exchange.result.dayPlan.length && exchange.result.places.length ? (
                <View style={styles.inlineCard}><Text style={styles.inlineTitle}>Good options nearby</Text><Text style={styles.inlineText}>{exchange.result.places.length} places matched. Ask me to build a day around one, make it easier, or surprise you.</Text></View>
              ) : null}
              {isLatest && exchange.result?.followUps.length && !confirmed ? (
                <View style={styles.chipWrap}>{exchange.result.followUps.slice(0, 4).map((follow) => <Pressable key={follow} style={styles.choiceChip} onPress={() => handleFollowUp(follow)}><Text style={styles.choiceText}>{shortFollowUp(follow)}</Text></Pressable>)}</View>
              ) : null}
            </View>
          );
        })}

        {plannerStage === 'duration' ? <View style={styles.inlineCard}><Text style={styles.inlineTitle}>How much time do you have?</Text><View style={styles.chipWrap}>{DURATION_CHOICES.map((item) => <Pressable key={item} style={styles.choiceChip} onPress={() => chooseDuration(item)}><Text style={styles.choiceText}>{item}</Text></Pressable>)}</View></View> : null}
        {plannerStage === 'vibe' ? <View style={styles.inlineCard}><Text style={styles.inlineTitle}>What sounds good?</Text><View style={styles.chipWrap}>{VIBE_CHOICES.map((item) => <Pressable key={item} style={styles.choiceChip} onPress={() => chooseVibe(item)}><Text style={styles.choiceText}>{item}</Text></Pressable>)}</View></View> : null}
        {renderFoodChoices()}
        <View style={styles.bottomClearance} />
      </ScrollView>

      <View style={styles.composerShell}>
        {exchanges.length > 0 ? <Pressable onPress={startFresh}><Text style={styles.freshText}>Start fresh</Text></Pressable> : null}
        <View style={styles.composer}>
          <TextInput value={query} onChangeText={setQuery} placeholder={confirmed ? 'Ask about this outing…' : planning ? 'Change this plan…' : 'Ask Go anything…'} placeholderTextColor="#7C8981" style={styles.input} multiline maxLength={500} onSubmitEditing={() => void ask()} />
          <Pressable disabled={busy || query.trim().length < 3} onPress={() => void ask()} style={[styles.sendButton, (busy || query.trim().length < 3) && styles.sendDisabled]}><AppIcon name="arrow-up" color="#152019" size={18} /></Pressable>
        </View>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  smallButton: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 12, backgroundColor: '#243329' },
  smallButtonText: { color: '#C9D1CC', fontSize: 10.5, fontWeight: '700' },
  ...
  confirmButton: { marginTop: 10, minHeight: 38, borderRadius: 12, backgroundColor: '#D7B45A', alignItems: 'center', justifyContent: 'center' },
  confirmText: { color: '#172017', fontSize: 12, fontWeight: '900' },
  ...
  freshText: { alignSelf: 'flex-end', color: '#8D9A92', fontSize: 10.5, fontWeight: '700', marginBottom: 5 },
