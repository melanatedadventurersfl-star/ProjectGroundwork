...
export default function TrailGuideScreen() {
  ...
  useEffect(() => {
    ...
    void getWeatherByQuery(`${cityName}, FL`)
      ...
      .finally(() => {
        if (active) setWeatherBusy(false);
      });
    return () => {
      active = false;
    };
  ...
  const cityPlaces = useMemo(
    () => trailGuidePlaces.filter((place) => place.city === cityKey),
    [cityKey],
  );
    ...
    void Promise.all(
      unresolved.map(async (place) => {
        try {
          ...
          return [
            place.id,
            distanceMiles(coordinates.latitude, coordinates.longitude, match.latitude, match.longitude),
          ] as const;
        } catch {
          return null;
        }
  ...
  const rankedCityPlaces = useMemo(() => {
    return [...cityPlaces].sort((a, b) => {
      const conditionDelta =
        getTrailGuideConditionSignal(b, weather).score - getTrailGuideConditionSignal(a, weather).score;
  ...
  const filteredPlaces = useMemo(() => {
    if (category === 'All') return rankedCityPlaces;
    return rankedCityPlaces.filter((place) => place.category === category);
  ...
  function renderRecommendedCard(place: TrailGuidePlace) {
    ...
    return (
      ...
      >
        <ImageBackground source={{ uri: place.image }} style={styles.recommendedImage} imageStyle={styles.recommendedImageRadius}>
  ...
  function renderExploreCard(place: TrailGuidePlace) {
    const signal = getTrailGuideConditionSignal(place, weather);
    const distance = formatDistance(place);
    return (
      <Pressable
      ...
      >
        ...
        <View style={styles.exploreCopy}>
          ...
          <View style={styles.exploreMetaRow}>
            <Text style={styles.exploreType}>{place.type}</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.page}>
        <ImageBackground source={backgroundSource} style={styles.hero} imageStyle={styles.heroImage}>
          ...
          <View style={styles.heroContent}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [styles.locationRow, pressed && styles.chipPressed]}
            >
        ...
        <View style={styles.body}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recommendedRow}>
            {recommendedPlaces.map(renderRecommendedCard)}
          </ScrollView>
            ...
            {primaryGuideArticles.map((guide) => (
              ...
              >
                ...
                <View style={styles.guideCopy}>
                  ...
                  <Text style={styles.guideAction}>Start here</Text>
                </View>
              </Pressable>
            ))}
          </ScrollView>
          ...
          <View style={styles.exploreSectionHeader}>
            <View style={styles.sectionHeaderCopy}>
              <View style={styles.exploreTitleRow}>
                <Text style={styles.sectionTitle}>Explore {cityName}</Text>
                <Text style={styles.dynamicCount}>{filteredPlaces.length} {categoryLabel}</Text>
              </View>
              <Text style={styles.sectionSubtitle}>Curated outdoor places around the city.</Text>
            </View>
          </View>

          {explorePlaces.length > 0 ? (
            <View style={styles.exploreGrid}>{explorePlaces.map(renderExploreCard)}</View>
          ) : (
          ...
          {filteredPlaces.length > EXPLORE_PREVIEW_LIMIT ? (
            ...
            >
              <Text style={styles.seeAllText}>
                {showAll ? 'Show less' : `See all ${filteredPlaces.length} ${categoryLabel}`}
              </Text>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { marginTop: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between' },
  ...
  recommendedImage: { height: 118, justifyContent: 'flex-end', padding: 9 },
  recommendedImageRadius: { borderTopLeftRadius: 15, borderTopRightRadius: 15 },
  cardShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: 'rgba(4,9,6,0.24)' },
  ...
  exploreImage: { width: 112, minHeight: 108, backgroundColor: '#1B241F' },
