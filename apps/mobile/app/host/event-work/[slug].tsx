...
import { attentionScore, campaignDateAssessment, campaignProgress, isOverdue, needsAttention, needsScheduling, openTasksForCampaign, taskTiming, type WorkTask } from '../../../src/hosting/workModel';
...
export default function EventWorkScreen() {
  ...
  const dateAssessment = campaignDateAssessment(campaign);

  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <Pressable onPress={() => router.back()}><Text style={styles.back}>‹ My Work</Text></Pressable>
    <View style={styles.header}><View style={[styles.accent, { backgroundColor: campaign.accent || '#D7B45A' }]} /><Text style={styles.kicker}>EVENT WORK</Text><Text style={styles.title}>{campaign.shortTitle}</Text><Text style={styles.date}>{new Date(campaign.startsAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</Text><View style={styles.metrics}><Metric value={tasks.length} label="Open" /><Metric value={overdue} label="Overdue" /><Metric value={critical} label="Critical" /><Metric value={blocked} label="Blocked" /></View><Text style={styles.progress}>{campaignProgress(campaign)}% task completion · {scheduling} need scheduling</Text></View>

    {dateAssessment.state === 'review' ? <View style={styles.warning}><Text style={styles.warningTitle}>Review event dates</Text><Text style={styles.warningText}>{dateAssessment.reason}</Text></View> : null}
