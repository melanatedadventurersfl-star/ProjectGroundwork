...
import { attentionScore, filterTasks, flattenOpenTasks, needsAttention, taskTiming, type WorkFilter, type WorkTask } from '../../src/hosting/workModel';
...
export default function HostWorkListScreen() {
  ...
  const tasks = useMemo(() => {
    let next = flattenOpenTasks(campaigns);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.kicker}>TASKS</Text><Text style={styles.title}>{params.filter === 'attention' ? 'Needs Attention' : params.event ? 'Event Tasks' : 'All Open Tasks'}</Text><Text style={styles.meta}>{tasks.length} task{tasks.length === 1 ? '' : 's'} in this view</Text>
