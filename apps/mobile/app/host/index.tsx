...
const COLORS = {
  bg: '#09100C', panel: '#121A15', raised: '#19231D', line: '#2A382F', cream: '#FFF8E8',
  muted: '#A2ADA5', dim: '#718077', gold: '#DDBB59', green: '#79D28C', danger: '#F06F65',
  blue: '#7DB8EF', orange: '#E8A15B', purple: '#AB92EA',
};
...
type PerformanceSummary = { adventureId: string; ticketsSold: number; revenueCents: number; capacity: number | null };
type SummaryTone = 'green' | 'gold' | 'blue' | 'danger';

export default function HostCenterScreen() {
  const { width } = useWindowDimensions();
  const roomy = width >= 760;
  const desktop = width >= 1024;
  const cardWidth = desktop ? 330 : Math.max(270, Math.min(width * 0.86, 310));
      ...
      if (adventureIds.length > 0) {
        try {
          ...
          for (const adventureId of adventureIds) {
            ...
            if (ticketsSold > 0 || revenueCents > 0 || capacity !== null) nextPerformance.set(adventureId, { adventureId, ticketsSold, revenueCents, capacity });
          }
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    >
      <View style={styles.heroShell}>
        ...
        <View style={styles.topbar}>
          ...
          <View style={styles.topActions}>
            <Pressable style={styles.createButton} onPress={() => router.push('/host/create' as never)}>
              <AppIcon name="add" color="#171B16" size={18} /><Text style={styles.createButtonText}>Build Event</Text>
            </Pressable>
        ...
        {approved ? <View style={[styles.summaryStrip, roomy && styles.summaryStripRoomy]}>
          <Summary value={String(active.length)} label="Active events" meta={`${active.filter((item) => getCampaignDaysUntil(item.campaign) > 0).length} upcoming`} icon="calendar" tone="green" onPress={() => router.push('/host/events' as never)} roomy={roomy} />
          <Summary value={formatCountdown(nextEventDays)} label="Next event" meta={nextEvent ? compactEventTitle(nextEvent.campaign.shortTitle) : 'No event scheduled'} icon="time" tone="gold" onPress={() => nextEvent ? router.push(`/host/campaigns/${nextEvent.campaign.slug}` as never) : router.push('/host/events' as never)} roomy={roomy} />
          <Summary value={String(openTasks.length)} label="Open tasks" meta={`${overdueTasks.length} overdue`} icon="tasks" tone="blue" onPress={() => router.push('/host/work' as never)} roomy={roomy} />
          <Summary value={String(attentionCount)} label="Need attention" meta={`${overdueTasks.length} overdue · ${blockedCount} blocked`} icon="notifications" tone="danger" onPress={() => router.push('/host/work' as never)} roomy={roomy} />
        </View> : null}
      </View>
      ...
      {approved ? <>
        ...
        <SectionHeader title="Active events" subtitle="Your events at a glance" action="View all" onPress={() => router.push('/host/events' as never)} />
        ...
        {recommendedTask ? <Pressable style={styles.nextActionCard} onPress={() => router.push('/host/work' as never)}>
          <View style={styles.nextActionIcon}><AppIcon name="sparkles" color={COLORS.gold} size={19} /></View>
          ...
          <View style={styles.nextActionButton}><Text style={styles.nextActionButtonText}>View task</Text><Text style={styles.nextActionButtonArrow}>›</Text></View>
        </Pressable> : null}

        <View style={[styles.board, roomy && styles.boardRoomy]}>
          <View style={[styles.boardColumn, roomy && styles.boardColumnRoomy]}>
            <SectionHeader title="Needs attention" badge={attentionCount > 0 ? String(attentionCount) : undefined} action={attentionCount > 2 ? 'View all' : undefined} onPress={attentionCount > 2 ? () => router.push('/host/work' as never) : undefined} />
            ...
            <View style={styles.listCard}>
              {attentionTasks.length === 0 && pendingVendors === 0 ? <Text style={styles.emptyText}>Nothing needs attention right now.</Text> : <>
                ...
                {attentionTasks.length < 2 && pendingVendors > 0 ? <Pressable style={[styles.alertRow, attentionTasks.length > 0 && styles.divider]} onPress={() => router.push('/host/vendors' as never)}><View style={[styles.alertIcon, { backgroundColor: `${COLORS.orange}18` }]}><AppIcon name="message" color={COLORS.orange} size={17} /></View><View style={{ flex: 1, minWidth: 0 }}><Text style={styles.alertTitle}>{pendingVendors} vendor response{pendingVendors === 1 ? '' : 's'} pending</Text><Text style={styles.alertMeta}>Vendor activity · Waiting</Text></View><Text style={styles.chevron}>›</Text></Pressable> : null}
              </>}
            </View>
          </View>

          <View style={[styles.boardColumn, roomy && styles.boardColumnRoomy]}>
            <SectionHeader title="My work" badge={myTasks.length > 0 ? String(myTasks.length) : undefined} action={myTasks.length > 3 ? 'View all' : undefined} onPress={myTasks.length > 3 ? () => router.push('/host/work' as never) : undefined} />
            <View style={styles.listCard}>
        ...
        <View style={[styles.lowerGrid, roomy && styles.lowerGridRoomy]}>
          <View style={[styles.lowerColumn, roomy && styles.lowerColumnRoomy]}>
            <View style={[styles.sectionStandaloneHeader, styles.operationsHeader]}><Text style={styles.sectionTitleStandalone}>Operations</Text><Text style={styles.scopeLabel}>Across active events</Text></View>
            <View style={styles.businessGrid}>
              ...
              <BusinessCard title="Vendors" value={confirmedVendors > 0 ? `${confirmedVendors} confirmed` : 'Clear'} meta={`${pendingVendors} pending`} icon="storefront" accent={COLORS.blue} route="/host/vendors" />
...
function TopographicBackdrop() {
  return <View pointerEvents="none" style={styles.backdrop}>
    ...
    {[0, 1, 2, 3].map((index) => <View key={`a-${index}`} style={[styles.contour, { width: 170 + index * 40, height: 115 + index * 30, top: -30 + index * 10, right: -20 - index * 6, opacity: 0.16 - index * 0.025 }]} />)}
    {[0, 1, 2].map((index) => <View key={`b-${index}`} style={[styles.contour, styles.contourSecondary, { width: 120 + index * 45, height: 85 + index * 26, bottom: 18 - index * 10, left: -40 - index * 8, opacity: 0.1 - index * 0.02 }]} />)}
  </View>;
}
...
function SectionHeader({ title, subtitle, badge, action, onPress }: { title: string; subtitle?: string; badge?: string; action?: string; onPress?: () => void }) {
  return <View style={styles.sectionHeader}><View style={{ flex: 1 }}><View style={styles.sectionTitleRow}><Text style={styles.sectionTitle}>{title}</Text>{badge ? <View style={styles.sectionBadge}><Text style={styles.sectionBadgeText}>{badge}</Text></View> : null}</View>{subtitle ? <Text style={styles.sectionSubtitle}>{subtitle}</Text> : null}</View>{action && onPress ? <Pressable onPress={onPress}><Text style={styles.sectionAction}>{action}</Text></Pressable> : null}</View>;
}

function Summary({ value, label, meta, icon, tone, onPress, roomy }: { value: string; label: string; meta: string; icon: Parameters<typeof AppIcon>[0]['name']; tone: SummaryTone; onPress: () => void; roomy: boolean }) {
  const accent = tone === 'green' ? COLORS.green : tone === 'gold' ? COLORS.gold : tone === 'blue' ? COLORS.blue : COLORS.danger;
  const background = tone === 'green' ? '#10271A' : tone === 'gold' ? '#2A2310' : tone === 'blue' ? '#122332' : '#311515';
  return <Pressable style={[styles.summaryCard, { borderColor: `${accent}88`, backgroundColor: background }, roomy && styles.summaryCardRoomy]} onPress={onPress}>
    <Text style={[styles.summaryArrow, { color: accent }]}>›</Text>
    <View style={styles.summaryContent}>
      <View style={[styles.summaryIcon, { backgroundColor: `${accent}22`, borderColor: `${accent}55` }]}><AppIcon name={icon} color={accent} size={18} /></View>
      <Text style={styles.summaryValue} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82}>{value}</Text>
      <Text style={styles.summaryLabel} numberOfLines={1}>{label}</Text>
      <Text style={[styles.summaryMeta, { color: accent }]} numberOfLines={1}>{meta}</Text>
    </View>
  </Pressable>;
}
...
function EventCard({ campaign, operations, heroImageUrl, width }: { campaign: HostCampaign; operations: Awaited<ReturnType<typeof getEventOperationsSummary>>; heroImageUrl: string | null; width: number }) {
  ...
  const statusLabel = days <= 0 ? 'HAPPENING TODAY' : campaign.status.toUpperCase();
  const statusAccent = days <= 0 ? COLORS.gold : campaign.status === 'planning' ? COLORS.muted : COLORS.blue;
  const art = <View style={styles.eventImageOverlay}><View style={[styles.eventStatusChip, { backgroundColor: statusAccent }]}><Text style={styles.eventStatus}>{statusLabel}</Text></View><Text style={styles.eventTitle} numberOfLines={2}>{compactEventTitle(campaign.shortTitle)}</Text><Text style={styles.eventLocation} numberOfLines={1}>{formatDisplayLocation(campaign.location)}</Text></View>;
  return <Pressable style={[styles.eventCard, { width }]} onPress={() => router.push(`/host/campaigns/${campaign.slug}` as never)}>
    {heroImageUrl ? <ImageBackground source={{ uri: heroImageUrl }} style={styles.eventArt} imageStyle={styles.eventImage} resizeMode="cover">{art}</ImageBackground> : <View style={[styles.eventArt, styles.eventArtFallback, { backgroundColor: campaign.accent || '#26352B' }]}>{art}</View>}
    <View style={styles.eventBody}><View style={styles.eventProgressLine}><Text style={styles.eventReady}>{readinessLabel}</Text><Text style={styles.eventPercent}>{operations.progress}%</Text></View><View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${Math.max(0, Math.min(operations.progress, 100))}%`, backgroundColor: operations.progress >= 60 ? COLORS.green : COLORS.gold }]} /></View><View style={styles.eventMetrics}><Metric icon="tasks" text={`${remaining} tasks`} /><Metric icon="notifications" text={`${attention} attention`} danger={attention > 0} /><Metric icon="calendar" text={days <= 0 ? 'Today' : `${date.toLocaleDateString(undefined, { month: 'short' })} ${date.getDate()}`} /></View></View>
  </Pressable>;
}

function Metric({ icon, text, danger = false }: { icon: Parameters<typeof AppIcon>[0]['name']; text: string; danger?: boolean }) {
  return <View style={styles.metricItem}><AppIcon name={icon} color={danger ? COLORS.danger : COLORS.cream} size={13} /><Text style={[styles.metricText, danger && styles.danger]}>{text}</Text></View>;
}
...
function compactEventTitle(value: string) { if (/dubious advice/i.test(value)) return 'Dubious Advice'; if (value.length <= 42) return value; const parts = value.split(':').map((part) => part.trim()).filter(Boolean); const candidate = parts[1]; if (candidate && candidate.length >= 5 && candidate.length <= 42) return candidate; return `${value.slice(0, 39).trim()}…`; }
...
const styles = StyleSheet.create({
  ...
  skeletonContent: { padding: 18 }, skeleton: { backgroundColor: COLORS.raised, borderRadius: 8 }, skeletonGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 20 }, skeletonMetric: { width: '48.5%', height: 112, borderRadius: 16 },

  heroShell: { position: 'relative', overflow: 'hidden', marginHorizontal: -18, marginTop: -18, paddingHorizontal: 18, paddingTop: 20, paddingBottom: 16, backgroundColor: '#0C1A12', borderBottomWidth: 1, borderBottomColor: '#1F3326' },
  backdrop: { ...StyleSheet.absoluteFillObject, overflow: 'hidden' }, backdropGlow: { position: 'absolute', width: 340, height: 240, borderRadius: 180, backgroundColor: '#183A25', opacity: 0.28, top: -100, left: -70 },
  contour: { position: 'absolute', borderWidth: 1, borderColor: '#DDBB59', borderRadius: 999 }, contourSecondary: { borderColor: '#79D28C' },
  topbar: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }, eyebrow: { color: COLORS.gold, fontSize: 9, fontWeight: '900', letterSpacing: 1.6 }, title: { color: COLORS.cream, fontSize: 32, fontWeight: '900', marginTop: 3 }, subtitle: { color: COLORS.muted, fontSize: 11.5, lineHeight: 16, marginTop: 3, maxWidth: 520 },
  topActions: { flexDirection: 'row', gap: 8 }, createButton: { minHeight: 46, borderRadius: 13, backgroundColor: COLORS.gold, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 6 }, createButtonText: { color: '#171B16', fontSize: 11, fontWeight: '900' }, menuButton: { width: 46, height: 46, borderRadius: 13, backgroundColor: '#142018', borderWidth: 1, borderColor: '#314238', alignItems: 'center', justifyContent: 'center' },

  summaryStrip: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, marginTop: 8 }, summaryStripRoomy: { flexWrap: 'nowrap' }, summaryCard: { width: '48.5%', minHeight: 156, borderRadius: 17, borderWidth: 1, padding: 12, position: 'relative', alignItems: 'center', justifyContent: 'center' }, summaryCardRoomy: { width: 'auto', flex: 1, minWidth: 145, minHeight: 164 }, summaryContent: { width: '100%', alignItems: 'center', justifyContent: 'center', paddingHorizontal: 8 }, summaryIcon: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, alignItems: 'center', justifyContent: 'center', marginBottom: 15 }, summaryArrow: { position: 'absolute', top: 11, right: 12, fontSize: 22, lineHeight: 24, fontWeight: '700', zIndex: 2 }, summaryValue: { width: '100%', color: COLORS.cream, fontSize: 25, lineHeight: 30, fontWeight: '900', textAlign: 'center' }, summaryLabel: { width: '100%', color: COLORS.cream, fontSize: 10.5, marginTop: 4, textAlign: 'center' }, summaryMeta: { width: '100%', fontSize: 8.5, marginTop: 12, fontWeight: '700', textAlign: 'center' }, danger: { color: COLORS.danger },

  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 20, marginBottom: 8 }, sectionTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 7 }, sectionTitle: { color: COLORS.cream, fontSize: 19, fontWeight: '900' }, sectionSubtitle: { color: COLORS.muted, fontSize: 9.5, marginTop: 2 }, sectionBadge: { minWidth: 24, height: 24, borderRadius: 12, backgroundColor: COLORS.danger, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 6 }, sectionBadgeText: { color: '#fff', fontSize: 9, fontWeight: '900' }, sectionAction: { color: COLORS.gold, fontSize: 10, fontWeight: '900' },

  eventRow: { gap: 12, paddingRight: 18 }, eventCard: { minHeight: 232, borderRadius: 18, backgroundColor: '#0F1713', borderWidth: 1, borderColor: '#34443A', overflow: 'hidden' }, eventArt: { height: 142, justifyContent: 'flex-end' }, eventArtFallback: { height: 112 }, eventImage: { borderTopLeftRadius: 17, borderTopRightRadius: 17 }, eventImageOverlay: { flex: 1, justifyContent: 'flex-end', padding: 12, backgroundColor: 'rgba(6,10,8,0.22)' }, eventStatusChip: { alignSelf: 'flex-start', borderRadius: 999, paddingHorizontal: 9, paddingVertical: 5, marginBottom: 'auto' }, eventStatus: { color: '#121612', fontSize: 7.5, fontWeight: '900', letterSpacing: .7 }, eventTitle: { color: '#fff', fontSize: 17, lineHeight: 20, fontWeight: '900', textShadowColor: 'rgba(0,0,0,.7)', textShadowRadius: 4 }, eventLocation: { color: '#F3EDE0', fontSize: 9.2, marginTop: 2, textShadowColor: 'rgba(0,0,0,.7)', textShadowRadius: 4 }, eventBody: { padding: 12 }, eventProgressLine: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, eventReady: { color: COLORS.cream, fontSize: 9.5 }, eventPercent: { color: COLORS.cream, fontSize: 10, fontWeight: '800' }, progressTrack: { height: 5, borderRadius: 5, backgroundColor: '#2A332E', marginTop: 7, overflow: 'hidden' }, progressFill: { height: '100%', borderRadius: 5 }, eventMetrics: { marginTop: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 8 }, metricItem: { flexDirection: 'row', alignItems: 'center', gap: 4 }, metricText: { color: COLORS.muted, fontSize: 8.8 },

  nextActionCard: { marginTop: 16, minHeight: 82, borderRadius: 17, borderWidth: 1, borderColor: '#6A5521', backgroundColor: '#151810', padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10 }, nextActionIcon: { width: 39, height: 39, borderRadius: 12, backgroundColor: '#2B2617', alignItems: 'center', justifyContent: 'center' }, nextActionEyebrow: { color: COLORS.gold, fontSize: 7.8, fontWeight: '900', letterSpacing: .8 }, nextActionTitle: { color: COLORS.cream, fontSize: 12.5, fontWeight: '900', marginTop: 2 }, nextActionMeta: { color: COLORS.muted, fontSize: 8.8, marginTop: 2 }, nextActionButton: { minHeight: 38, borderRadius: 11, backgroundColor: COLORS.gold, paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', gap: 5 }, nextActionButtonText: { color: '#171B16', fontSize: 9, fontWeight: '900' }, nextActionButtonArrow: { color: '#171B16', fontSize: 18, fontWeight: '900' },

  board: { gap: 0 }, boardRoomy: { flexDirection: 'row', gap: 14 }, boardColumn: {}, boardColumnRoomy: { flex: 1 }, attentionSummary: { minHeight: 30, justifyContent: 'center', paddingHorizontal: 4 }, attentionSummaryText: { color: COLORS.muted, fontSize: 9.3 }, listCard: { borderRadius: 16, backgroundColor: COLORS.panel, borderWidth: 1, borderColor: COLORS.line, overflow: 'hidden' }, emptyText: { color: COLORS.dim, fontSize: 11, padding: 14 }, alertRow: { minHeight: 58, paddingHorizontal: 11, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 9 }, alertIcon: { width: 34, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, alertTitle: { color: COLORS.cream, fontSize: 11.5, fontWeight: '900' }, alertMeta: { color: COLORS.dim, fontSize: 8.8, marginTop: 2 },
  taskRow: { minHeight: 56, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'stretch', gap: 9 }, priorityBar: { width: 4, borderRadius: 4, marginVertical: 11 }, taskTitle: { color: COLORS.cream, fontSize: 11.8, fontWeight: '900', marginTop: 9 }, taskMeta: { color: COLORS.dim, fontSize: 8.8, marginTop: 2, marginBottom: 9 }, clearState: { padding: 12 }, clearTitle: { color: COLORS.cream, fontSize: 12, fontWeight: '900' }, clearMeta: { color: COLORS.dim, fontSize: 9, marginTop: 2 }, quickAddRow: { minHeight: 42, margin: 8, borderRadius: 11, backgroundColor: '#211D12', paddingHorizontal: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 }, quickAddText: { color: COLORS.gold, fontSize: 10.5, fontWeight: '900' }, divider: { borderTopWidth: 1, borderTopColor: COLORS.line }, chevron: { color: COLORS.muted, fontSize: 22, alignSelf: 'center' },

  lowerGrid: { gap: 0 }, lowerGridRoomy: { flexDirection: 'row', alignItems: 'flex-start', gap: 14 }, lowerColumn: {}, lowerColumnRoomy: { flex: 1 }, sectionStandaloneHeader: { marginTop: 18, marginBottom: 7 }, operationsHeader: { alignItems: 'center' }, sectionTitleStandalone: { color: COLORS.cream, fontSize: 18, fontWeight: '900' }, scopeLabel: { color: COLORS.dim, fontSize: 8.5, marginTop: 2 }, businessGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, businessCard: { width: '48.5%', minHeight: 104, borderRadius: 15, backgroundColor: COLORS.panel, borderWidth: 1, borderColor: COLORS.line, padding: 14, alignItems: 'center', justifyContent: 'center' }, businessIcon: { width: 30, height: 30, borderRadius: 9, alignItems: 'center', justifyContent: 'center', marginBottom: 9 }, businessTitle: { width: '100%', color: COLORS.cream, fontSize: 11.5, fontWeight: '900', textAlign: 'center' }, businessValue: { width: '100%', color: COLORS.cream, fontSize: 17, fontWeight: '900', marginTop: 4, textAlign: 'center' }, businessMeta: { width: '100%', color: COLORS.dim, fontSize: 8.8, lineHeight: 12, marginTop: 2, textAlign: 'center' },
  ...
  zeroState: { marginTop: 18, borderRadius: 18, borderWidth: 1, borderColor: COLORS.line, backgroundColor: COLORS.panel, padding: 18 }, zeroIcon: { width: 46, height: 46, borderRadius: 14, backgroundColor: '#2B2617', alignItems: 'center', justifyContent: 'center' }, zeroTitle: { color: COLORS.cream, fontSize: 18, fontWeight: '900', marginTop: 12 }, zeroBody: { color: COLORS.muted, fontSize: 11, lineHeight: 17, marginTop: 5, maxWidth: 560 }, emptyEvent: { width: 280, minHeight: 140, borderRadius: 17, borderWidth: 1, borderColor: COLORS.line, backgroundColor: COLORS.panel, padding: 16, justifyContent: 'center' }, emptyEventTitle: { color: COLORS.cream, fontSize: 15, fontWeight: '900' }, emptyEventBody: { color: COLORS.dim, fontSize: 10, marginTop: 4 },
  accessCard: { marginTop: 12, borderRadius: 18, backgroundColor: COLORS.panel, borderWidth: 1, borderColor: COLORS.line, padding: 16 }, accessTitle: { color: COLORS.cream, fontSize: 17, fontWeight: '900' }, accessBody: { color: COLORS.muted, fontSize: 11, lineHeight: 16, marginTop: 5 }, primary: { alignSelf: 'flex-start', marginTop: 12, minHeight: 42, borderRadius: 11, backgroundColor: COLORS.gold, justifyContent: 'center', paddingHorizontal: 13 }, primaryText: { color: '#172017', fontSize: 11, fontWeight: '900' }, errorCard: { marginTop: 10, borderRadius: 14, backgroundColor: '#281915', borderWidth: 1, borderColor: '#66362D', padding: 12 }, error: { color: '#FFB4A9', fontSize: 11 }, retry: { color: COLORS.gold, fontSize: 10, fontWeight: '900', marginTop: 6 },
});