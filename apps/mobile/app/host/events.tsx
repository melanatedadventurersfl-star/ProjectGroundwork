...
type EventRow = { campaign: HostCampaign; operations: Awaited<ReturnType<typeof getEventOperationsSummary>> };
...
export default function HostEventsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const campaigns = (await listHostCampaigns()).filter((campaign) => campaign.status !== 'complete');
      const rows = await Promise.all(campaigns.map(async (campaign) => ({ campaign, operations: await getEventOperationsSummary(campaign.id) })));
      setItems(rows);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Text style={styles.eyebrow}>EVENTS</Text><Text style={styles.title}>Events</Text><Text style={styles.subtitle}>Build, run and adjust every event. The Event Assistant reads the live event state, tasks and connected analytics before it answers.</Text>
    ...
    {items.map(({ campaign, operations }) => <View key={campaign.id} style={styles.card}>
      ...
      <View style={styles.stats}><Text style={styles.stat}>{operations.overdueTaskCount} overdue</Text><Text style={styles.stat}>{campaign.tasks.filter((task) => task.status !== 'complete').length} open tasks</Text><Text style={styles.stat}>${(operations.profitCents / 100).toLocaleString()} profit</Text></View>
      <View style={styles.actions}><Pressable style={styles.secondary} onPress={() => router.push(`/host/manage/${campaign.adventureId}` as never)}><Text style={styles.secondaryText}>Open Event</Text></Pressable><Pressable style={styles.ai} onPress={() => router.push(`/host/assistant/${campaign.adventureId}` as never)}><Text style={styles.aiText}>✦ Event Assistant</Text></Pressable></View>
    </View>)}
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#0A0F0C' }, content: { padding: 18, paddingBottom: 72 }, back: { color: '#D7B45A', fontSize: 11, fontWeight: '900', marginBottom: 17 }, eyebrow: { color: '#D7B45A', fontSize: 9, fontWeight: '900', letterSpacing: 1.1 }, title: { color: '#FFF8E8', fontSize: 31, fontWeight: '900', marginTop: 3 }, subtitle: { color: '#92A097', fontSize: 11, lineHeight: 17, marginTop: 5 }, primary: { minHeight: 46, borderRadius: 13, backgroundColor: '#D7B45A', alignItems: 'center', justifyContent: 'center', marginTop: 14 }, primaryText: { color: '#172017', fontSize: 11, fontWeight: '900' }, loading: { paddingVertical: 35, alignItems: 'center', gap: 8 }, muted: { color: '#7F8B83', fontSize: 10 }, error: { color: '#FF9D92', fontSize: 10, marginTop: 12 }, empty: { borderRadius: 16, borderWidth: 1, borderColor: '#2D3932', backgroundColor: '#141B16', padding: 18, marginTop: 14 }, emptyTitle: { color: '#FFF8E8', fontSize: 14, fontWeight: '900' }, emptyBody: { color: '#849087', fontSize: 10, lineHeight: 15, marginTop: 4 }, card: { borderRadius: 17, borderWidth: 1, borderColor: '#2D3932', backgroundColor: '#141B16', padding: 14, marginTop: 11 }, cardTop: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, cardKicker: { color: '#D7B45A', fontSize: 8, fontWeight: '900', letterSpacing: .8 }, cardTitle: { color: '#FFF8E8', fontSize: 15, fontWeight: '900', marginTop: 2 }, cardMeta: { color: '#859188', fontSize: 9, marginTop: 4 }, ready: { color: '#E7C464', fontSize: 20, fontWeight: '900' }, track: { height: 4, borderRadius: 3, backgroundColor: '#29332D', overflow: 'hidden', marginTop: 12 }, fill: { height: 4, backgroundColor: '#D7B45A' }, stats: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 9 }, stat: { color: '#77847B', fontSize: 8 }, actions: { flexDirection: 'row', gap: 8, marginTop: 12 }, secondary: { flex: 1, minHeight: 42, borderRadius: 11, borderWidth: 1, borderColor: '#3B473F', alignItems: 'center', justifyContent: 'center' }, secondaryText: { color: '#D4DDD7', fontSize: 10, fontWeight: '900' }, ai: { flex: 1, minHeight: 42, borderRadius: 11, borderWidth: 1, borderColor: '#5C4B80', backgroundColor: '#1B1724', alignItems: 'center', justifyContent: 'center' }, aiText: { color: '#C6B0F4', fontSize: 10, fontWeight: '900' } });
