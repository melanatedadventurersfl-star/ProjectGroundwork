...
const visibilityOptions: Array<{ value: EventVisibility; label: string; copy: string }> = [
  ...
  { value: 'unlisted', label: 'Unlisted', copy: 'People need the direct link to open it.' },
  { value: 'private', label: 'Private', copy: 'Only invited people can view it.' },
  { value: 'community', label: 'Community', copy: 'Only members of selected communities can view it.' },
];
...
export default function CreateHostOutingScreen() {
  ...
  async function createOuting() {
    ...
    try {
      ...
      if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end <= start) throw new Error('Add valid start and end dates, with the end after the start.');
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.eyebrow}>BUILD MANUALLY</Text><Text style={styles.title}>Event basics</Text><Text style={styles.subtitle}>Enter the foundation yourself. AI planning is a separate guided path.</Text>
    ...
    <Text style={styles.helper}>Enter a normal date and time. The app validates it before creating the event.</Text>
    ...
    <Pressable disabled={saving} style={styles.primary} onPress={() => void createOuting()}>{saving ? <ActivityIndicator color="#172017" /> : <Text style={styles.primaryText}>Create Event</Text>}</Pressable>
