...
export default function ImportEventScreen() {
  ...
  async function analyze() {
    ...
    try {
      ...
      if (fileMode && inputMode === 'upload') {
        next = await uploadAndPreviewHostImport(selectedFiles.map((file) => ({
          uri: file.uri,
          name: file.name,
        ...
        next = await previewHostImport({
          mode,
          sourceUrl: mode === 'pasted_text' ? undefined : sourceUrl,
  ...
  async function create() {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Unable to create this imported event.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {result && draft && counts ? <>
      <View style={styles.sourceCard}><Text style={styles.kicker}>SOURCE PACKAGE</Text><Text style={styles.sourceTitle}>{result.sourceLabel}</Text><Text style={styles.sourceMeta}>{result.extractionSource === 'ai' ? 'Structured extraction complete' : 'Basic extraction only'}</Text></View>
      ...
      <View style={styles.warning}><Text style={styles.warningTitle}>Imported ticket details are reference only</Text><Text style={styles.warningText}>The event starts with a $0 General Admission shell. Configure and approve actual ticket tiers before publishing.</Text></View>
      {error ? <Text style={styles.error}>{error}</Text> : null}
