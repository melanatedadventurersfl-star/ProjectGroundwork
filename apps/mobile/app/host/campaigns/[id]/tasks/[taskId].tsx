...
export default function CampaignTaskDetailScreen() {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.header}><Pressable onPress={() => router.back()}><Text style={styles.back}>‹ {campaign.shortTitle}</Text></Pressable><Text style={styles.headerLabel}>TASK</Text></View><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {timingState === 'review' ? <View style={styles.reviewCard}><Text style={styles.label}>DATE NEEDS REVIEW</Text><Text style={styles.reviewText}>{task.dueLabel || new Date(task.dueAt as string).toLocaleDateString()}</Text><Text style={styles.reviewMeta}>This date is outside the supported event planning window and does not count as overdue.</Text></View> : null}
    <View style={styles.ownerCard}><Text style={styles.label}>PLAN OWNER</Text><Text style={styles.value}>{task.owner}</Text></View>
    {task.blockedBy ? <View style={styles.blocker}><Text style={styles.label}>DEPENDENCY</Text><Text style={styles.blockerText}>Blocked by {task.blockedBy}</Text></View> : task.status === 'blocked' ? <View style={styles.blocker}><Text style={styles.label}>BLOCKED</Text><Text style={styles.blockerText}>Blocked reason not recorded</Text></View> : null}
    {campaign.canManage ? <View style={styles.controls}><Control label="Status" value={statusLabel} onPress={() => setPicker('status')} /><Control label="Assigned To" value={assignee?.displayName ?? 'Unassigned'} onPress={() => setPicker('assignee')} /><Control label="Priority" value={task.priority} onPress={() => setPicker('priority')} /><Control label="Due Date" value={dueValue} onPress={() => { setDueInput(task.dueAt ? new Date(task.dueAt).toISOString().slice(0, 10) : ''); setPicker('due'); }} /></View> : null}
