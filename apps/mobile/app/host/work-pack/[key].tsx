...
export default function HostWorkPackScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const next = canonicalCampaigns((await listHostCampaigns()).filter((campaign) => campaign.status !== 'complete'));
      setCampaigns(next);
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.screen}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    {!loading && !error ? <>
      ...
      {missingCount === 0 ? <View style={styles.covered}><Text style={styles.coveredTitle}>{pack.shortTitle} plan is covered</Text><Text style={styles.coveredText}>All {assessment.length} recommended task{assessment.length === 1 ? '' : 's'} are complete or already in My Work.</Text></View> : null}
      <View style={styles.sectionHead}><View><Text style={styles.sectionTitle}>{selectedCampaign?.shortTitle ?? 'Event'} plan</Text><Text style={styles.sectionMeta}>{missingCount === 0 ? 'No missing work' : `${selectedKeys.length} missing selected`}</Text></View>{missingCount > 0 ? <View style={styles.links}><Pressable onPress={() => setSelectedKeys(assessment.filter((item) => item.state === 'missing').map((item) => item.key))}><Text style={styles.link}>All</Text></Pressable><Pressable onPress={() => setSelectedKeys([])}><Text style={styles.link}>Clear</Text></Pressable></View> : null}</View>
...
const styles = StyleSheet.create({
  ...
  covered: { marginTop: 12, borderRadius: 14, borderWidth: 1, borderColor: '#3F7255', backgroundColor: '#14251A', padding: 13 }, coveredTitle: { color: '#9FD2AC', fontSize: 11, fontWeight: '900' }, coveredText: { color: '#7FA28A', fontSize: 8, lineHeight: 12, marginTop: 3 },
