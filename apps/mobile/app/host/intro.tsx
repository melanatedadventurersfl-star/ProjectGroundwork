...
function renderStep(
  ...
  setProfile: (value: HostCenterProfile | null) => void,
  privacy: AiPrivacyPreferences,
  setPrivacy: (value: AiPrivacyPreferences) => void,
  finish: (destination: string) => Promise<void>,
) {
  if (!profile) return null;

  if (step === 1) return <>
    <Text style={styles.eyebrow}>WELCOME TO HOST CENTER</Text>
    <Text style={styles.title}>Your event workspace starts here.</Text>
    <Text style={styles.body}>Plan, organize, promote and run events without moving through the member app first.</Text>
    <View style={styles.grid}>
      <Feature title="Events" text="Build and manage every event." />
      <Feature title="Work" text="Tasks, owners and deadlines." />
      <Feature title="Calendar" text="Operational dates in one view." />
      <Feature title="Vendors" text="Businesses, documents and follow-up." />
      <Feature title="Marketing" text="Promotions and channel performance." />
      <Feature title="Finances" text="Revenue, expenses and event profit." />
    </View>
  </>;

  if (step === 2) return <>
    <Text style={styles.eyebrow}>HOW IT WORKS</Text>
    <Text style={styles.title}>One event, one operating system.</Text>
    <Text style={styles.body}>Host Center follows the event from the first idea through post-event review.</Text>
    <View style={styles.lifecycle}>
      {['Idea', 'Build Event', 'Work Plan', 'Promote', 'Run Event', 'Review'].map((label, index) => <View key={label} style={styles.lifecycleRow}><View style={styles.number}><Text style={styles.numberText}>{index + 1}</Text></View><Text style={styles.lifecycleText}>{label}</Text></View>)}
    </View>
    <Text style={styles.note}>Your member profile and Host Center share one Go Melanated account. Host permissions control what operational data you can access.</Text>
  </>;

  if (step === 3) return <>
    <Text style={styles.eyebrow}>HOST PROFILE</Text>
    <Text style={styles.title}>Set up how you operate.</Text>
    <Text style={styles.body}>These details help identify your organization and prefill future Host Center work. Optional fields can be completed later.</Text>
    <Field label="Organization or business name" value={profile.organizationName} onChangeText={(organizationName) => setProfile({ ...profile, organizationName })} placeholder="Melanated Adventurers" />
    <Field label="Host display name" value={profile.hostDisplayName} onChangeText={(hostDisplayName) => setProfile({ ...profile, hostDisplayName })} placeholder="Host name" />
    <View style={styles.twoCol}><View style={styles.flex}><Field label="City" value={profile.city} onChangeText={(city) => setProfile({ ...profile, city })} placeholder="Jacksonville" /></View><View style={styles.state}><Field label="State" value={profile.state} onChangeText={(state) => setProfile({ ...profile, state })} placeholder="FL" /></View></View>
    <Field label="Host contact email" value={profile.contactEmail} onChangeText={(contactEmail) => setProfile({ ...profile, contactEmail })} placeholder="events@example.com" keyboardType="email-address" />
    <Field label="Website" value={profile.websiteUrl} onChangeText={(websiteUrl) => setProfile({ ...profile, websiteUrl })} placeholder="https://" autoCapitalize="none" />
    <Field label="Public description" value={profile.publicDescription} onChangeText={(publicDescription) => setProfile({ ...profile, publicDescription })} placeholder="What should members know about your organization?" multiline />
    <Toggle label="Show public host profile" value={profile.publicProfileEnabled} onValueChange={(publicProfileEnabled) => setProfile({ ...profile, publicProfileEnabled })} />
  </>;

  if (step === 4) return <>
    <Text style={styles.eyebrow}>YOUR ROLE</Text>
    <Text style={styles.title}>What do you usually handle?</Text>
    <Text style={styles.body}>Choose the areas that fit your work. These are preferences, not permissions.</Text>
    <View style={styles.chips}>{WORKING_AREAS.map((area) => {
      const active = profile.workingAreas.includes(area);
      return <Pressable key={area} onPress={() => setProfile({ ...profile, workingAreas: active ? profile.workingAreas.filter((item) => item !== area) : [...profile.workingAreas, area] })} style={[styles.chip, active && styles.chipActive]}><Text style={[styles.chipText, active && styles.chipTextActive]}>{area}</Text></Pressable>;
    })}</View>
  </>;

  if (step === 5) return <>
    <Text style={styles.eyebrow}>AI & PRIVACY</Text>
    <Text style={styles.title}>You control what the AI remembers.</Text>
    <Text style={styles.body}>The Event Planner and Event Assistant can work without long-term personalization. Optional settings stay off unless you enable them.</Text>
    <Toggle label="Personal Memory" detail="Remember explicit host preferences for future planning." value={privacy.personal_memory_enabled} onValueChange={(value) => setPrivacy({ ...privacy, personal_memory_enabled: value })} />
    <Toggle label="Learn From Event History" detail="Use your previous events for future recommendations." value={privacy.event_history_learning_enabled} onValueChange={(value) => setPrivacy({ ...privacy, event_history_learning_enabled: value })} />
    <Toggle label="Shared Organization Memory" detail="Use approved organization knowledge in recommendations." value={privacy.organization_memory_enabled} onValueChange={(value) => setPrivacy({ ...privacy, organization_memory_enabled: value })} />
    <Toggle label="Save AI Planning Conversations" value={privacy.save_conversations_enabled} onValueChange={(value) => setPrivacy({ ...privacy, save_conversations_enabled: value })} />
    <Toggle label="Recommendation History" value={privacy.recommendation_history_enabled} onValueChange={(value) => setPrivacy({ ...privacy, recommendation_history_enabled: value })} />
    <Toggle label="Product Improvement Analytics" detail="Optional structured usage analytics. Raw planning conversations are not required for this setting." value={privacy.product_analytics_enabled} onValueChange={(value) => setPrivacy({ ...privacy, product_analytics_enabled: value })} />
  </>;

  return <>
    <Text style={styles.eyebrow}>READY TO START</Text>
    <Text style={styles.title}>Choose what you want to do first.</Text>
    <Text style={styles.body}>Connections can be added later. Eventbrite, Facebook, Instagram, email and calendar integrations will only show as connected after the related service is configured.</Text>
    <View style={styles.connections}>{['Eventbrite', 'Facebook', 'Instagram', 'Email', 'Calendar'].map((service) => <View key={service} style={styles.connection}><Text style={styles.connectionName}>{service}</Text><Text style={styles.connectionStatus}>NOT CONNECTED</Text></View>)}</View>
    <Action title="Plan an event with AI" text="Start with an idea and let the planner build the operational plan." onPress={() => void finish('/host/plan-ai')} />
    <Action title="Build an event manually" text="Enter the event foundation yourself." onPress={() => void finish('/host/create-scratch')} />
    <Action title="Import an existing event" text="Use a URL, file or existing event source." onPress={() => void finish('/host/create')} />
    <Action title="Explore Host Center" text="Open the dashboard and look around first." onPress={() => void finish('/host')} />
  </>;
}
