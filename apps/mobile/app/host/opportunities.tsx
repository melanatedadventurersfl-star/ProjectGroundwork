...
export default function HostOpportunitiesScreen() {
  ...
  async function importOpportunity() {
    ...
    try {
      ...
      if (existing) setNotice(`Already saved in ${existing.stage === 'saved' ? 'Saved' : 'Pipeline'}. Review the new details below and refresh the saved copy if needed.`);
  ...
  async function saveDiscovery(event: DiscoveredOpportunity & { sourceId:string; sourceLabel:string }, track = false) {
    try {
      ...
      else { setTab('saved'); setNotice(`Saved “${displayText(row.title)}”. It is now in your Saved opportunities.`); }
  ...
  async function savePreview(tags: string[], track = false) {
    ...
    try {
      ...
      else { setTab('saved'); setNotice(`Saved “${displayText(row.title)}”. You are viewing it in Saved now.`); }
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to save this opportunity.'); }
  }
  ...
  async function refreshPreview() {
    ...
    try {
      ...
      setError('');
      const destination: PageTab = row.stage === 'saved' ? 'saved' : 'pipeline';
      setTab(destination);
      setNotice(`Updated “${displayText(row.title)}” with the latest imported details. Your tags and pipeline stage were kept.`);
  ...
  function scheduleOuting(event: { title:string; summary?:string; startsAt?:string | null; endsAt?:string | null; venueName?:string; city?:string; state?:string; sourceUrl:string; organizer?:string; ticketUrl?:string }) {
    ...
    params.set('fromOpportunity','1'); params.set('title',displayText(event.title)); params.set('summary',displayText(event.summary || '')); params.set('startsAt',event.startsAt || ''); params.set('endsAt',event.endsAt || ''); params.set('venueName',displayText(event.venueName || '')); params.set('city',event.city || ''); params.set('state',event.state || 'FL'); params.set('sourceUrl',event.sourceUrl); params.set('organizer',displayText(event.organizer || '')); params.set('ticketUrl',event.ticketUrl || '');
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    {error ? <View style={styles.messageError}><Text style={styles.error}>{error}</Text></View> : null}{notice ? <View style={styles.messageSuccess}><Text style={styles.notice}>{notice}</Text></View> : null}
    ...
    {tab === 'saved' ? <OpportunityList items={saved.filter((item) => item.stage === 'saved')} empty="No saved opportunities yet." onSchedule={scheduleOuting} onTrack={async (item) => { await setOpportunityStage(item.id,'reviewing'); await refreshSaved(); setTab('pipeline'); setNotice(`Moved “${displayText(item.title)}” to Reviewing.`); }} onTags={persistTags} /> : null}
...
function ImportedCard({ preview,sourceLabel,existing,onSave,onTrack,onRefresh,onSchedule }:{ preview:OpportunityPreview; sourceLabel:string; existing:SavedOpportunity | null; onSave:(tags:string[])=>void; onTrack:(tags:string[])=>void; onRefresh:()=>void; onSchedule:()=>void }) {
  ...
  const rows = [
    ['Type',labelize(preview.opportunityType)],['Starts',formatDate(preview.eventStart)],['Ends',formatDate(preview.eventEnd)],['Venue',displayText(preview.venueName)],['Address',displayText(preview.address)],['City / State',[preview.city,preview.state].filter(Boolean).join(', ')],['Organizer',displayText(preview.organizer)],['Ticket / price',preview.ticketDetails?.join(' · ')],['Vendor fee',preview.vendorFeeText],['Application deadline',formatDate(preview.applicationDeadline)],['Contact',preview.contactName || preview.contactEmail || preview.contactPhone],
  ].filter(([,value]) => Boolean(value));
  return <View style={styles.reviewCard}>
    {preview.imageUrl ? <Image source={{ uri:preview.imageUrl }} style={styles.previewImage} resizeMode="cover" /> : null}
    <View style={styles.badges}><Badge text={preview.sourceUrl.includes('eventbrite.') ? 'Eventbrite sourced' : sourceLabel} />{existing ? <Badge text={`Already in ${existing.stage === 'saved' ? 'Saved' : 'Pipeline'}`} /> : <Badge text="Review before saving" />}</View>
    <Text style={styles.reviewTitle}>{displayText(preview.title || 'Imported opportunity')}</Text>{preview.summary ? <Text style={styles.cardBody}>{displayText(preview.summary)}</Text> : null}
    <View style={styles.details}>{rows.map(([label,value],index) => <View key={label} style={[styles.detailRow,index > 0 && styles.detailDivider]}><Text style={styles.detailLabel}>{label}</Text><Text style={styles.detailValue}>{displayText(String(value))}</Text></View>)}</View>
    ...
    <View style={styles.actionPanel}><Text style={styles.actionEyebrow}>{existing ? 'THIS OPPORTUNITY IS ALREADY SAVED' : 'WHAT DO YOU WANT TO DO?'}</Text><Text style={styles.actionCopy}>{existing ? 'Refresh the saved record with the newly imported details. Your tags and pipeline stage stay intact.' : 'Save it for later, actively pursue it, or turn the outside event into a meetup.'}</Text>
      {existing ? <Pressable style={styles.saveButton} onPress={onRefresh}><Text style={styles.saveButtonText}>Refresh saved details</Text></Pressable> : <Pressable style={styles.saveButton} onPress={() => onSave(tags)}><Text style={styles.saveButtonText}>Save opportunity</Text></Pressable>}
      <View style={styles.actions}>{!existing ? <SmallAction label="Track in pipeline" onPress={() => onTrack(tags)} /> : null}<SmallAction label="Schedule outing" onPress={onSchedule} featured /><SmallAction label="View original" onPress={() => void Linking.openURL(preview.sourceUrl)} /></View>
    </View>
  </View>;
}

function DiscoveryCard({ event,onSave,onTrack,onSchedule }:{ event:DiscoveredOpportunity & { sourceId:string; sourceLabel:string }; onSave:()=>void; onTrack:()=>void; onSchedule:()=>void }) { return <View style={styles.card}><View style={styles.badges}><Badge text={event.sourceLabel} /><RelevanceBadge value={event.relevanceLabel} /></View><Text style={styles.cardTitle}>{displayText(event.title)}</Text><Text style={styles.cardMeta}>{[formatDate(event.startsAt),displayText(event.venueName),event.city,event.state].filter(Boolean).join(' · ')}</Text>{event.summary ? <Text style={styles.cardBody}>{displayText(event.summary)}</Text> : null}{event.relevanceBasis ? <Text style={styles.basis}>Why it is marked: {event.relevanceBasis}</Text> : null}<ActionRow onOpen={() => void Linking.openURL(event.sourceUrl)} onSave={onSave} onTrack={onTrack} onSchedule={onSchedule} /></View>; }
...
function ReviewList({ title,items }:{ title:string; items:string[] }) { return <View style={styles.reviewList}><Text style={styles.reviewListTitle}>{title.toUpperCase()}</Text>{items.slice(0,8).map((item,index) => <Text key={`${title}-${index}`} style={styles.reviewListItem}>• {displayText(item)}</Text>)}</View>; }
