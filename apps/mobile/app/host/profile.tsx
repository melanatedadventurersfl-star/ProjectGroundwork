...
const C = { bg: '#0A0F0C', panel: '#131B16', raised: '#19231C', line: '#2D3A32', cream: '#FFF8E8', muted: '#95A29A', dim: '#6F7D75', gold: '#D7B45A', green: '#84C992' };
...
export default function HostProfileScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [p, o, h] = await Promise.all([getMyHostProfile(), listMyHostOrganizations(), listMyHostHistory()]);
      setProfile(p); setOrganizations(o); setHistory(h);
  ...
  async function saveProfile() {
    ...
    try {
      await updateMyHostProfile({ publicTitle: title, businessName, bio, websiteUrl: website, isPublic: true });
  ...
  async function addOrganization() {
    setSaving(true); setError('');
    try {
      await createHostOrganization({ name: newOrgName, description: newOrgDescription, city: profile?.homeCity || undefined, state: profile?.homeState || undefined });
      setNewOrgName(''); setNewOrgDescription(''); await load();
  ...
  async function addPhoto() {
    ...
    setSaving(true); setError('');
    try {
      await addHostHistoryPhoto(photoEventId, photoUrl, photoCaption);
      setPhotoEventId(null); setPhotoUrl(''); setPhotoCaption(''); await load();
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to add photo.'); }
    finally { setSaving(false); }
  }
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.hero}>
      ...
      <View style={styles.stats}><Stat value={String(profile?.followerCount ?? 0)} label="Followers" /><Stat value={String(completed.length)} label="Hosted" /><Stat value={String(upcoming.length)} label="Upcoming" /><Stat value={String(organizations.length)} label="Organizations" /></View>
    </View>
    ...
    <SectionHeader title="Organizations & businesses" meta="Hosting does not require a community group. Create or join an organization only when you want events presented under a shared public identity." />
    {organizations.map((org) => <View key={org.id} style={styles.orgCard}><View style={styles.orgLogo}><AppIcon name="storefront" color={C.gold} size={22} /></View><View style={{ flex: 1 }}><Text style={styles.orgName}>{org.name}</Text><Text style={styles.orgMeta}>{[org.city, org.state].filter(Boolean).join(', ') || 'Public host organization'}</Text>{org.description ? <Text style={styles.orgDescription}>{org.description}</Text> : null}</View></View>)}
    ...
    <SectionHeader title="Hosting history" meta="Past events become your public hosting story. Every event can carry its own photo gallery." />
    {completed.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>No completed events yet</Text><Text style={styles.muted}>Completed host events will appear here automatically.</Text></View> : completed.map((item) => <HistoryCard key={item.id} item={item} onAddPhoto={() => setPhotoEventId(item.id)} />)}

    {photoEventId ? <View style={styles.card}><Text style={styles.cardTitle}>Add a history photo</Text><Text style={styles.muted}>Attach a photo to this hosted event. This first release accepts an existing hosted image URL and stores it in the shared host media system.</Text><Field label="Photo URL" value={photoUrl} onChangeText={setPhotoUrl} placeholder="https://…" /><Field label="Caption" value={photoCaption} onChangeText={setPhotoCaption} placeholder="Opening night at camp" /><View style={styles.actionRow}><Pressable style={styles.cancel} onPress={() => setPhotoEventId(null)}><Text style={styles.cancelText}>Cancel</Text></Pressable><Pressable disabled={saving || !photoUrl.trim()} style={[styles.primary, styles.actionPrimary]} onPress={() => void addPhoto()}><Text style={styles.primaryText}>Add Photo</Text></Pressable></View></View> : null}

    <SectionHeader title="Upcoming events" meta="Events can be public, unlisted, private, or limited to selected communities." />
    {upcoming.map((item) => <View key={item.id} style={styles.upcomingCard}><View style={{ flex: 1 }}><Text style={styles.eventTitle}>{item.title}</Text><Text style={styles.eventMeta}>{formatDate(item.starts_at)} · {item.city}, {item.state}</Text></View><View style={styles.visibilityPill}><Text style={styles.visibilityText}>{item.visibility.toUpperCase()}</Text></View></View>)}
  </ScrollView></SafeAreaView>;
}

function HistoryCard({ item, onAddPhoto }: { item: HostHistoryItem; onAddPhoto: () => void }) {
  return <View style={styles.historyCard}><View style={styles.historyTop}><View style={{ flex: 1 }}><Text style={styles.eventTitle}>{item.title}</Text><Text style={styles.eventMeta}>{formatDate(item.starts_at)} · {item.city}, {item.state}</Text></View><Pressable style={styles.photoButton} onPress={onAddPhoto}><AppIcon name="photo" color={C.gold} size={16} /><Text style={styles.photoButtonText}>Add photo</Text></Pressable></View>{item.host_media?.length ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.photoRow}>{item.host_media.map((photo) => <View key={photo.id} style={styles.photoWrap}><Image source={{ uri: photo.image_url }} style={styles.photo} />{photo.caption ? <Text style={styles.photoCaption} numberOfLines={2}>{photo.caption}</Text> : null}</View>)}</ScrollView> : <Text style={styles.noPhotos}>No photos added yet.</Text>}</View>;
}
