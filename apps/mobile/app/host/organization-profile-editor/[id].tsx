...
export default function HostOrganizationProfileEditorScreen() {
  ...
  const hydrate = useCallback((nextOrg:HostOrganization,nextSetup:HostProfileSetupData) => {
    ...
    setDescription(nextOrg.description || ''); setCity(nextOrg.city || ''); setState(nextOrg.state || ''); setServiceAreas(nextSetup.serviceAreas.join(', '));
    setWebsite(nextOrg.website_url || ''); setEmail(nextOrg.public_email || ''); setPhone(nextOrg.phone || ''); setInstagram(nextOrg.instagram_url || ''); setFacebook(nextOrg.facebook_url || '');
    setSpecialties((nextOrg.specialties || []).join(', ')); setAudiences(nextSetup.audiences.join(', ')); setLanguages(nextSetup.languages.join(', ')); setAccessibility(nextSetup.accessibility || '');
    setFoundedYear(nextSetup.foundedYear ? String(nextSetup.foundedYear) : ''); setIsPublic(nextOrg.is_public); setSectionOrder(nextSetup.profileSectionOrder.length ? nextSetup.profileSectionOrder : DEFAULT_ORDER); setContactVisibility(nextSetup.contactVisibility);
  ...
  const load = useCallback(async() => {
    ...
    try {
      ...
      setOrg(nextOrg); setSetup(nextSetup); setGallery(nextGallery); hydrate(nextOrg,nextSetup);
    } catch (caught) {
  ...
  async function save(stage:HostSetupStage = step) {
    ...
    try {
      ...
      setOrg(nextOrg); setSetup(nextSetup); hydrate(nextOrg,nextSetup); setSaved(stage === 'complete' ? 'Profile setup complete.' : 'Profile progress saved.');
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to save this host profile.'); }
  ...
  async function chooseBrandImage(kind:'logo'|'cover') {
    ...
    try {
      if (kind === 'logo') await uploadOrganizationLogo(id,result.assets[0].uri); else await uploadOrganizationCover(id,result.assets[0].uri);
      await load();
  ...
  async function chooseGalleryPhotos() {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to upload gallery photos.'); }
    finally { setSaving(false); }
  }
  ...
  function confirmDeletePhoto(photo:OrganizationGalleryPhoto) {
    Alert.alert('Remove photo?','This removes the photo from the host gallery.',[
      ...
      { text:'Remove', style:'destructive', onPress:()=>void (async()=>{ try { await deleteOrganizationGalleryPhoto(photo.id); if(id)setGallery(await listOrganizationGallery(id)); } catch(caught){ setError(caught instanceof Error?caught.message:'Unable to remove photo.'); } })() },
    ]);
  }

  async function featurePhoto(photo:OrganizationGalleryPhoto) {
    if (!id) return;
    try { await updateOrganizationGalleryPhoto(photo.id,{ isFeatured:true }); setGallery(await listOrganizationGallery(id)); }
    catch(caught){ setError(caught instanceof Error?caught.message:'Unable to feature photo.'); }
  }
  ...
  async function analyzeImport() {
    ...
    try {
      const result = await previewHostProfileImport({
        organizationId:id, mode:importMode,
    ...
    } catch(caught){ setError(caught instanceof Error?caught.message:'Unable to read those profile details.'); }
  ...
  async function applyImport() {
    ...
    try {
      const corePatch:any = {};
      if(has('name'))corePatch.name=p.name; if(has('tagline'))corePatch.tagline=p.tagline; if(has('description'))corePatch.description=p.description;
      if(has('city'))corePatch.city=p.city; if(has('state'))corePatch.state=p.state; if(has('websiteUrl'))corePatch.website_url=p.websiteUrl;
      if(has('publicEmail'))corePatch.public_email=p.publicEmail; if(has('phone'))corePatch.phone=p.phone; if(has('instagramUrl'))corePatch.instagram_url=p.instagramUrl;
      if(has('facebookUrl'))corePatch.facebook_url=p.facebookUrl; if(has('specialties'))corePatch.specialties=p.specialties;
      const setupPatch:any = {};
      if(has('hostType')&&p.hostType)setupPatch.hostType=p.hostType; if(has('shortDescription'))setupPatch.shortDescription=p.shortDescription;
      if(has('serviceAreas'))setupPatch.serviceAreas=p.serviceAreas; if(has('audiences'))setupPatch.audiences=p.audiences; if(has('languages'))setupPatch.languages=p.languages;
      if(has('accessibility'))setupPatch.accessibility=p.accessibility; if(has('foundedYear'))setupPatch.foundedYear=p.foundedYear;
      ...
      setImportResult(null); setSelectedImportFields([]); setImportFiles([]); setImportText(''); setImportUrl(''); await load(); setSaved('Imported details applied. Review and save any edits.');
    } catch(caught){ setError(caught instanceof Error?caught.message:'Unable to apply imported details.'); }
    finally { setImportBusy(false); }
  }

  async function discardImport() {
    if(importResult) await markHostProfileImportDiscarded(importResult.importId).catch(()=>undefined);
    setImportResult(null); setSelectedImportFields([]);
  }
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    <View style={styles.top}><Pressable onPress={()=>router.back()}><Text style={styles.back}>‹ Host Center</Text></Pressable><Pressable style={styles.previewButton} onPress={()=>router.push(`/organization-profile/${org.slug}` as never)}><Text style={styles.previewButtonText}>View as visitor</Text></Pressable></View>

    <Text style={styles.eyebrow}>HOST PROFILE</Text><Text style={styles.title}>Build the identity people will see.</Text><Text style={styles.subtitle}>Add your information once. Use the same profile for setup, future edits, discovery, and events.</Text>

    <View style={styles.progressCard}><View style={styles.progressTop}><Text style={styles.progressValue}>{completion.percent}%</Text><View style={styles.flex}><Text style={styles.progressTitle}>Profile quality</Text><Text style={styles.muted}>{completion.complete} of {completion.total} recommended items complete</Text></View></View><View style={styles.progressTrack}><View style={[styles.progressFill,{width:`${completion.percent}%`}]} /></View>{completion.missing.slice(0,3).map((item)=><Text key={item} style={styles.missing}>• {item}</Text>)}</View>

    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.steps}>{STEPS.map((item,index)=><Pressable key={item.key} style={[styles.step,index===currentIndex&&styles.stepActive]} onPress={()=>void goTo(item.key)}><Text style={[styles.stepNumber,index===currentIndex&&styles.stepNumberActive]}>{index+1}</Text><Text style={[styles.stepLabel,index===currentIndex&&styles.stepLabelActive]}>{item.label}</Text></Pressable>)}</ScrollView>

    {error?<View style={styles.errorCard}><Text style={styles.error}>{error}</Text></View>:null}{saved?<View style={styles.savedCard}><Text style={styles.saved}>{saved}</Text></View>:null}

    {step==='profile'?<>
      <Section title="Identity" copy="Start with the name and type of host people should recognize."/>
      <View style={styles.card}><Field label="Public host name" value={name} onChangeText={setName} placeholder="Groundwork Test Company"/><Text style={styles.label}>Host type</Text><View style={styles.chips}>{HOST_TYPES.map((item)=><Pressable key={item.key} style={[styles.chip,hostType===item.key&&styles.chipActive]} onPress={()=>setHostType(item.key)}><Text style={[styles.chipText,hostType===item.key&&styles.chipTextActive]}>{item.label}</Text></Pressable>)}</View><Field label="Tagline" value={tagline} onChangeText={setTagline} placeholder="What should people know in one line?"/><Field label="Short description" value={shortDescription} onChangeText={setShortDescription} multiline placeholder="A short summary for cards and search results."/><View style={styles.row}><View style={styles.flex}><Field label="Home city" value={city} onChangeText={setCity}/></View><View style={styles.state}><Field label="State" value={state} onChangeText={(value:string)=>setState(value.toUpperCase())}/></View></View><Field label="Service areas" value={serviceAreas} onChangeText={setServiceAreas} placeholder="Jacksonville, Atlanta, Remote"/><Field label="Founded / started year" value={foundedYear} onChangeText={setFoundedYear} keyboardType="number-pad" placeholder="2024"/></View>
    ...
    {step==='about'?<>
      ...
      <View style={styles.card}><Field label="About" value={description} onChangeText={setDescription} multiline placeholder="Tell visitors what you host, offer, or organize."/><Field label="Specialties" value={specialties} onChangeText={setSpecialties} placeholder="Workshops, networking, family events"/><Field label="Typical audiences" value={audiences} onChangeText={setAudiences} placeholder="Families, professionals, beginners"/><Field label="Languages" value={languages} onChangeText={setLanguages} placeholder="English, Spanish"/><Field label="Accessibility information" value={accessibility} onChangeText={setAccessibility} multiline placeholder="Share confirmed accessibility details or accommodations."/></View>
      <Section title="Public contact" copy="Choose what visitors can use to reach or learn more about you."/>
      <View style={styles.card}><Field label="Website" value={website} onChangeText={setWebsite} autoCapitalize="none" placeholder="https://"/><Field label="Public email" value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"/><Field label="Phone" value={phone} onChangeText={setPhone} keyboardType="phone-pad"/><Field label="Instagram" value={instagram} onChangeText={setInstagram} autoCapitalize="none"/><Field label="Facebook" value={facebook} onChangeText={setFacebook} autoCapitalize="none"/></View>
    </>:null}
    ...
    <View style={styles.footer}><Pressable disabled={saving} style={styles.secondaryButton} onPress={()=>void save(step)}><Text style={styles.secondaryText}>{saving?'Saving…':'Save progress'}</Text></Pressable>{currentIndex<STEPS.length-1?<Pressable disabled={saving} style={styles.primaryButton} onPress={()=>void goTo(STEPS[currentIndex+1]!.key)}>{saving?<ActivityIndicator color="#152018"/>:<Text style={styles.primaryText}>Continue to {STEPS[currentIndex+1]!.label}</Text>}</Pressable>:<Pressable disabled={saving} style={styles.primaryButton} onPress={()=>void save('complete')}>{saving?<ActivityIndicator color="#152018"/>:<Text style={styles.primaryText}>Finish profile setup</Text>}</Pressable>}</View>
  </ScrollView></SafeAreaView>;
}

function hasImportValue(value:HostProfileImportPreview[keyof HostProfileImportPreview]) { if(Array.isArray(value))return value.length>0; return value!==null&&value!==''; }
function importValue(value:HostProfileImportPreview[keyof HostProfileImportPreview]) { if(Array.isArray(value))return value.join(', '); if(value===null)return ''; return String(value); }
function ImportPanel({mode,setMode,files,url,text,setUrl,setText,onChooseFiles,onAnalyze,busy,result,selected,setSelected,onApply,onDiscard}:any){return <><Section title="Bring your existing details" copy="Upload a media kit, profile document, flyer, website, or existing About copy. Nothing is applied until you review it."/><View style={styles.card}><View style={styles.modeRow}>{(['files','website','pasted_text'] as const).map((key)=><Pressable key={key} style={[styles.mode,mode===key&&styles.modeActive]} onPress={()=>setMode(key)}><Text style={[styles.modeText,mode===key&&styles.modeTextActive]}>{key==='files'?'Upload':key==='website'?'Website':'Paste'}</Text></Pressable>)}</View>{!result&&mode==='files'?<Pressable style={styles.importDrop} onPress={()=>void onChooseFiles()}><Text style={styles.uploadTitle}>{files.length?`${files.length} file${files.length===1?'':'s'} selected`:'Choose source files'}</Text><Text style={styles.muted}>PDF, Word, text, HTML, JPG, PNG, WebP</Text></Pressable>:null}{!result&&mode==='website'?<Field label="Public website or profile page" value={url} onChangeText={setUrl} autoCapitalize="none" placeholder="https://"/>:null}{!result&&mode==='pasted_text'?<Field label="Existing profile or business copy" value={text} onChangeText={setText} multiline placeholder="Paste your About text, services, contact details, service areas, or other public information."/>:null}{!result?<Pressable disabled={busy} style={styles.analyzeButton} onPress={()=>void onAnalyze()}>{busy?<ActivityIndicator color="#152018"/>:<Text style={styles.primaryText}>Review suggested details</Text>}</Pressable>:<><Text style={styles.importSource}>{result.sourceLabel} · {result.extractionSource}</Text>{result.preview.confidenceNotes?.map((note:string)=><Text key={note} style={styles.note}>• {note}</Text>)}<View style={styles.importList}>{IMPORT_FIELDS.filter(({key})=>hasImportValue(result.preview[key])).map(({key,label})=>{const active=selected.includes(key);return <Pressable key={key} style={[styles.importField,active&&styles.importFieldActive]} onPress={()=>setSelected((current:any[])=>current.includes(key)?current.filter((item)=>item!==key):[...current,key])}><View style={[styles.check,active&&styles.checkActive]}><Text style={styles.checkText}>{active?'✓':''}</Text></View><View style={styles.flex}><Text style={styles.importLabel}>{label}</Text><Text numberOfLines={3} style={styles.importValue}>{importValue(result.preview[key])}</Text></View></Pressable>})}</View><View style={styles.row}><Pressable style={[styles.secondaryButton,styles.flex]} onPress={()=>void onDiscard()}><Text style={styles.secondaryText}>Discard</Text></Pressable><Pressable disabled={busy||!selected.length} style={[styles.primaryButton,styles.flex,!selected.length&&styles.disabled]} onPress={()=>void onApply()}>{busy?<ActivityIndicator color="#152018"/>:<Text style={styles.primaryText}>Apply selected</Text>}</Pressable></View></>}</View></>}
...
function MediaTile({title,image,wide=false,onPress}:{title:string;image:string|null|undefined;wide?:boolean;onPress:()=>void}){return <Pressable style={[styles.mediaTile,wide&&styles.mediaTileWide]} onPress={onPress}>{image?<Image source={{uri:image}} style={styles.mediaTileImage}/>:<View style={styles.mediaTileEmpty}><Text style={styles.mediaTilePlus}>+</Text></View>}<Text style={styles.mediaTileTitle}>{title}</Text><Text style={styles.smallAction}>{image?'Replace':'Upload'}</Text></Pressable>}
function ToggleRow({label,value,onPress}:{label:string;value:boolean;onPress:()=>void}){return <Pressable style={styles.toggleRow} onPress={onPress}><Text style={styles.toggleLabel}>{label}</Text><View style={[styles.toggle,value&&styles.toggleOn]}><View style={[styles.toggleThumb,value&&styles.toggleThumbOn]}/></View></Pressable>}
